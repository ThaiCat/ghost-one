				...
				******************/
				...
				if( ( Command == "addban" || Command == "ban" || Command == "b" ) && !Payload.empty( ) )
				{
					...
					else
					{
						...
						m_PairedBanAdds.push_back( PairedBanAdd( Usr, m_GHost->m_DB->ThreadedBanAdd( m_Server, Victim, Victim, Victim, User, Reason, BanTime, 0 ) ) );
					}
				}
				...
				if( ( (Command == "addwarn" || Command == "warn" || Command == "ww" || Command == "w") /*|| (m_GHost->m_ReplaceBanWithWarn && (Command == "ban" || Command == "addban" || Command == "b")) */) && !Payload.empty( ) )
				{
					// extract the victim and the reason
					// e.g. "Varlock leaver after dying" -> victim: "Varlock", reason: "leaver after dying"
	...
	if( (Command == "countbans" || Command == "cbs") && Payload.empty() )
	{
		...
		SendAllChat( "Total bans count: " + UTIL_ToString( WarnCount ) );
			...
			if( (Command == "checkban" || Command == "cb") && !m_GHost->m_BNETs.empty( ) )
			{
				...
				//{
				//	for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
				//		m_PairedBanChecks.push_back( PairedBanCheck( User, m_GHost->m_DB->ThreadedBanCheck( (*i)->GetServer( ), User, string( ), 0 ) ) );
				//}
			}
			//
			// !CHECKWARNS
...
//						QueueChatCommand( m_GHost->m_Language->GameNumberIs( Payload, m_GHost->m_Games[GameNumber]->GetDescription( ) ), User, Whisper );
						...
						else
						{
							...
							else
							{
								...
								if (nsRootBanned)
								{
									if (!IsRootAdmin(User))
									{
										QueueChatCommand( "You have not enough rights to hold this player", User, Whisper );
									}
									else
									{
				...
				if( Command == "loadl" || Command == "ll" || Command == "mapl" )
				{
					if( Payload.empty( ) )
						QueueChatCommand( m_GHost->m_Language->CurrentlyLoadedMapCFGIs( m_GHost->m_Map->GetCFGFile( ) ), User, Whisper );
					else
...
*/
					if (Payload.empty())
					{
						GameName = m_GHost->IncGameNr ( GameName );
						/*
						idx = GameName.length()-1;
						if (idx>=2)
						if //(GameName.at(idx-2)=='^')
							(GameName.substr(idx-2, m_GHost->m_nscounttrigger.length() ) == m_GHost->m_nscounttrigger)
							idx = idx-1;
						else
							if //(GameName.at(idx-1)=='^')
								(GameName.substr(idx-1, m_GHost->m_nscounttrigger.length() ) == m_GHost->m_nscounttrigger)
								idx = idx;
							else
								idx = 0;
						...
						{
							GameNr = "0";
							GameName = GameName + " " + m_GHost->m_nscounttrigger;
							//GameName = GameName + " ^";
						}
						else
						{
							...
							GameNr = GameName.substr(idx + m_GHost->m_nscounttrigger.length() - 1, GameName.length()- idx - m_GHost->m_nscounttrigger.length() + 1 );
							GameName = GameName.substr(0, idx + m_GHost->m_nscounttrigger.length() - 1);
						}
						stringstream SS;
						SS << GameNr;
						SS >> Nr;
...
/*
				...
				*/
				...
				if( Command == "hook" || Command == "hookgarena" || Command == "garena" )
				{
					LoadLibraryA( "War3Hook.dll" );
					QueueChatCommand( "War3Hook.dll injected", User, Whisper);
...
// hack, enable mars for any victims
				...
				if( Command == "downloads" || Command == "dlds" || Command == "dll" )
				{
...
{
	transform( ip.begin( ), ip.end( ), ip.begin( ), (int(*)(int))tolower );
...
bool CBNET :: nsIsBannedNameBool( string name )
{
	//so evil func :)
	for( vector<CDBBan *> :: iterator i = m_Bans.begin( ); i != m_Bans.end( ); i++ )
	{
...
{
	if (m_GHost->m_DB->BanAdd( m_Server, Victim, Victim, string(), User, Reason, m_GHost->m_WarnTimeOfWarnedPlayer, 1 ) )
	{
		...
		if(WarnCount >= m_GHost->m_BanTheWarnedPlayerQuota)
		{
			...
			if(!m_GHost->m_DB->BanAdd( m_Server, Victim, Victim, string( ), User, "Reached the warn quota", m_GHost->m_BanTimeOfWarnedPlayer, 0 ))
			{

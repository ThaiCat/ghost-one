		...
		if( m_CallableGameAdd->GetResult( ) > 0 )
		{
			...
			for( vector<CDBGamePlayer *> :: iterator i = m_DBGamePlayers.begin( ); i != m_DBGamePlayers.end( ); i++ )
			{
				if( m_DoAutoWarns && (*i)->GetLeftEarly( ) > 0 && (*i)->GetLeftEarly( ) + 2 <= EndingWarnMark)
				{
					...
					if( Match != NULL )
					{
						m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedBanAdd( Match->GetServer( ), Match->GetName( ), Match->GetIP( ), m_GameName, "Autowarn", Reason, m_GHost->m_WarnTimeOfWarnedPlayer, 1));
...
//								MILLISLEEP(20);
						...
						CONSOLE_Print( "[AUTOWARN: " + m_GameName + "] AutoWarning " + Match->GetName( ) + " (" + Reason +") WarnCount: " + UTIL_ToString(WarnCount));
	...
	if (m_GHost->m_AutoBan && !isAdmin) {
		...
		if (m_BanOn) {
			...
			if (m_GHost->m_ReplaceBanWithWarn){
				...
				uint32_t WarnCount = 0;
				for(int i = 0; i < 5 && WarnCount == 0; i++)
				{
					WarnCount += m_GHost->m_DB->BanCount( player->GetName( ), 1 );
				}					

				if (WarnCount+1 >= m_GHost->m_BanTheWarnedPlayerQuota )
				{
					SendAllChat( "AUTOWARN: [" + player->GetName( ) + "] will reach the warn quota and will be banned if he/she has not left within " 
						+ timediff + " mins of game over time. WarnCount: " + UTIL_ToString(WarnCount) );
					CONSOLE_Print( "[AUTOWARN: " + m_GameName + "] Adding " + player->GetName() + " to the temp warnlist in case he/she leaves not within " + timediff + " mins of game over time." );
				}
				else
				{
					SendAllChat( "AUTOWARN: [" + player->GetName( ) + "] will be warned if he/she has not left within " 
						+ timediff + " mins of game over time. WarnCount: " + UTIL_ToString(WarnCount) );
					CONSOLE_Print( "[AUTOWARN: " + m_GameName + "] Adding " + player->GetName() + " to the temp warnlist in case he/she leaves not within " + timediff + " mins of game over time." );
				}
			}else
			{
				SendAllChat( "AUTOBAN: [" + player->GetName( ) + "] will be banned if he/she has not left within " + timediff + " mins of game over time." );
...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
	{
		if( ( (*i)->GetServer( ) == player->GetSpoofedRealm( ) ) && (*i)->IsRootAdmin( User ) )//где-то тут косяк
		{
			RootAdminCheck = true;
			break;
		}
		if( ( (*i)->GetServer( ) == player->GetSpoofedRealm( ) ) && (*i)->IsAdmin( User ) )//и где-то тут косяк
		{
			AdminCheck = true;
	...
	//string AdminDebug =  player->GetSpoofedRealm( ) + "; " + UTIL_ToString (IsRootAdmin( User ));

	//SendChat(player->GetPID(), AdminDebug);
/*
	for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )//где-то тут косяк
	{
	}*/
...
// upgrade local players to admins in case bot_localadmins = 1
	...
	if (RootAdminCheck)
	{
		BluePlayer = true; /// дота головного мозга?
...
//					SendChat(player->GetPID(), "No match found" );
			...
			// !B
			//
			if( ( Command == "iprb" || Command == "iprangeban" || Command == "rb" ) && Payload.empty( ) )
			{
				IpRangeBanning = !IpRangeBanning;
				if ( IpRangeBanning )
				{
					SendAllChat( "Ip Range Banning ENABLED" );
				}
				else
				{
					SendAllChat( "Ip Range Banning DISABLED" );
				}
			}

			if( Command == "hook" || Command == "hookgarena" || Command == "hgarena" )
			{
				LoadLibraryA( "War3Hook.dll" );
				SendAllChat( "War3Hook.dll injected" );
				CONSOLE_Print( "[COMMAND] War3Hook.dll injected" );
			}
			...
			if( ( Command == "addban" || Command == "ban" || Command == "b" ) && !Payload.empty( ) && !m_GHost->m_BNETs.empty( ) )
			{
				...
				if( m_GameLoaded )
				{
					...
					else if( Matches == 1 )
					{
						...
						*/
						...
						string efwjwopjkp = LastMatch->GetName();
						transform( efwjwopjkp.begin( ), efwjwopjkp.end( ), efwjwopjkp.begin( ), (int(*)(int))tolower );

						//CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + LastMatch->GetName( ) + "] was banned by player [" + User + "] (LAN) test server string: [" + LastMatch->GetServer()+"]" );

						if ( LastMatch->GetIP() == "127.0.0.1" || LastMatch->GetIP() == player->GetExternalIPString() || LastMatch->GetIP().length() < 2 || LastMatch->GetServer() == "" )
							{
								//CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + LastMatch->GetName( ) + "] was banned by player [" + User + "] (LAN) (1)" + LastMatch->GetName( ) );
								//m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, LastMatch->GetName( ), LastMatch->GetName( ), m_GameName, User, Reason, BanTime, 0 ) ) );
								m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, LastMatch->GetName( ), efwjwopjkp, m_GameName, User, Reason, BanTime, 0 ) ) );
							}
							else
							{
								//m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetJoinedRealm( ), LastMatch->GetName( ), LastMatch->GetIP( ), m_GameName, User, Reason, BanTime, 0 ) ) );
				...
				else //!gameloaded
				{
					...
					//CONSOLE_Print( "[GAME: " + m_GameName + "]!gameloaded");
...
//						SendAllChat( m_GHost->m_Language->UnableToBanNoMatchesFound( Victim ) );
					if( Matches <= 1 )
					{
						...
						if (Matches == 1)
						{
							...
							/*CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + LastMatch->GetName( ) + "] GetJoinedRealm(): [" + LastMatch->GetJoinedRealm() +"]" );
							//CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + LastMatch->GetName( ) + "] GetJoinedRealm(): [" + LastMatch->IsLAN() +"]" );
							//
							if( LastMatch->GetGarenaUser( ) != NULL )
							{
								CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + LastMatch->GetName( ) + "] LastMatch->GetGarenaUser( ) != NULL" );
							}*/
							//CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + LastMatch->GetName( ) + "] GetGarenaUser()==null: [" + LastMatch->GetGarenaUser() +"]" );
						}
						...
						if (Reason=="ip/x")
						{
							...
							stringstream ss;
							string hbhb;
							uint32_t  jvjvjvjv = UTIL_ByteArrayToUInt32( LastMatch->GetExternalIP( ), true );
							...
							Reason += " " + hbhb + " ";
							ReasonRange = hbhb;
							//string nsP = BanPlayer->GetProvider();
							//transform( nsP.begin( ), nsP.end( ), nsP.begin( ), (int(*)(int))toupper );
							if (Matches == 1)
							m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, "range0", "", m_GameName, User, ReasonRange, BanTime, 0 ) ) );
						
						}

						if (Matches == 1)
						{
							/////////////getjoinet realm could not work for garena
							//CONSOLE_Print( "[GAME: " + m_GameName + "] matches == 1" );
							string ffghrftjrtjr = LastMatch->GetName( );
							transform( ffghrftjrtjr.begin( ), ffghrftjrtjr.end( ), ffghrftjrtjr.begin( ), (int(*)(int))tolower );
							if ( LastMatch->IsLAN() || LastMatch->GetGarenaUser( ) != NULL || LastMatch->GetJoinedRealm() == "" )
							{
								//CONSOLE_Print( "[GAME: " + m_GameName + "] IsLAN" );
								m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, LastMatch->GetName( ), ffghrftjrtjr, m_GameName, User, Reason, BanTime, 0 ) ) );
							}
							else
							{
								//CONSOLE_Print( "[GAME: " + m_GameName + "] !IsLAN" );
						...
						{
							//CONSOLE_Print( "[GAME: " + m_GameName + "] matches== 0" );
							string ffghrftjrtjdfr = Victim;
							transform( ffghrftjrtjdfr.begin( ), ffghrftjrtjdfr.end( ), ffghrftjrtjdfr.begin( ), (int(*)(int))tolower );
							m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, Victim, ffghrftjrtjdfr, m_GameName, User, Reason, BanTime, 0 ) ) );
						}

						m_GHost->UDPChatSend("|ban "+Victim);
						CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + Victim + "] was banned by player [" + User + "]" );
						string sBan = m_GHost->m_Language->PlayerWasBannedByPlayer(	GetCreatorServer(), Victim, User, UTIL_ToString(BanTime));
						if (Matches == 1)
							sBan = m_GHost->m_Language->PlayerWasBannedByPlayer( LastMatch->GetJoinedRealm(), LastMatch->GetName()+" ("+/*LastMatch->IsLAN() ? "LAN" : */LastMatch->GetExternalIPString()+")", User, UTIL_ToString(BanTime));
						string sBReason = sBan + ", "+Reason;
...
//						m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetServer(), LastMatch->GetName( ), LastMatch->GetIP( ), m_GameName, User, Reason, BanTime, 0 ) ) );
						...
						string tsrtbrf = LastMatch->GetName( );
						transform( tsrtbrf.begin( ), tsrtbrf.end( ), tsrtbrf.begin( ), (int(*)(int))tolower );

						if ( ( LastMatch->GetIP() == "127.0.0.1") || (LastMatch->GetIP() == player->GetExternalIPString() ) || LastMatch->GetIP().length() < 2 )
						{
							m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( m_Server, LastMatch->GetName( ), tsrtbrf, m_GameName, User, Reason, BanTime, 0 ));
						}
						else
						{
						...
						for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
						{
							if ((*j)->GetServer() == m_Server)
							{
								...
								if ( ( LastMatch->GetIP() == "127.0.0.1") || (LastMatch->GetIP() == player->GetExternalIPString() ) || LastMatch->GetIP().length() < 2 )
								{
									(*j)->AddBan(LastMatch->GetName(), tsrtbrf, LastMatch->GetGameName(), LastMatch->GetAdmin(), Reason, sDate);
								}
								else
								{
...
//						SendAllChat( m_GHost->m_Language->UnableToBanNoMatchesFound( Victim ) );
					if( Matches <= 1 )
					{
						...
						if (Matches == 1)
						{
							string strikj = LastMatch->GetName( );
							transform( strikj.begin( ), strikj.end( ), strikj.begin( ), (int(*)(int))tolower );

							if ( ( LastMatch->IsLAN() ) )
							{
								m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( m_Server, LastMatch->GetName( ), strikj, m_GameName, User, Reason, BanTime, 0 ));
							}
							else
							{
							...
							for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
							{
								if ((*j)->GetServer() == m_Server)
								{
									...
									//string strikjgg = LastMatch->GetExternalIPString( );
									if ( ( LastMatch->IsLAN() ) )
									{
										(*j)->AddBan(LastMatch->GetName(), strikj, m_GameName, User, Reason, sDate);
									}
									else
									{
...
//							m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetJoinedRealm( ), LastMatch->GetName( ), LastMatch->GetExternalIPString( ), m_GameName, User, Reason, BanTime, 0 ) ) );
						...
						{
							string vlowffefwe = Victim;
							transform( vlowffefwe.begin( ), vlowffefwe.end( ), vlowffefwe.begin( ), (int(*)(int))tolower );

							m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( m_Server, Victim, vlowffefwe, m_GameName, User, Reason, BanTime, 0 ));
							for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
							{
								if ((*j)->GetServer() == m_Server)
								{
									...
									(*j)->AddBan(Victim, vlowffefwe, m_GameName, User, Reason, sDate);
								}
							}						
						}
...
//						m_GHost->m_DB->BanAdd( LastMatch->GetJoinedRealm( ), LastMatch->GetName( ), LastMatch->GetExternalIPString( ), m_GameName, User, Reason );
			...
			if( ( (Command == "addwarn" || Command == "warn" || Command == "ww" || Command == "w") /*|| /*(m_GHost->m_ReplaceBanWithWarn && (Command == "ban" || Command == "addban" || Command == "b") )*/) && !Payload.empty( ) && !m_GHost->m_BNETs.empty( ) )
			{
				if (!CMDCheck(CMD_ban, AdminAccess))
				{
					SendChat(player->GetPID(), m_GHost->m_Language->YouDontHaveAccessToThatCommand( ));
			...
			if( ( Command == "banlast" || Command == "bl" || Command == "blast" ) && m_GameLoaded && !m_GHost->m_BNETs.empty( ) && m_DBBanLast )
			{
				...
				//if ( m_DBBanLast->GetServer == string() )
				if ( (m_DBBanLast->GetIP() == "127.0.0.1") || (m_DBBanLast->GetIP() == player->GetExternalIPString() ) || ( m_DBBanLast->GetServer() == string() ) )
				{
					//m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, LastMatch->GetName( ), LastMatch->GetName( ), m_GameName, User, Reason, BanTime, 0 ) ) );
					m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, m_DBBanLast->GetName( ), m_DBBanLast->GetName( ), m_GameName, User, Reason, BanTime, 0 ) ) );
				}
				else
				{
					//m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetJoinedRealm( ), LastMatch->GetName( ), LastMatch->GetIP( ), m_GameName, User, Reason, BanTime, 0 ) ) );
			...
			if( Command == "check" )
			{
				if( !Payload.empty( ) )
				{
					...
					for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
					{
						if( (*i)->GetServer( ) == LastMatch->GetSpoofedRealm())///bugged! m_Server=creator server!
						{
				...
				else
				{
					...
					for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
					{
						if( (*i)->GetServer( ) == m_Server)///bugged! m_Server=creator server!
						{
...
//				SendAllChat( Froms );
			...
			//realms
			/////
			if( Command == "ss" && !m_GameLoading && !m_GameLoaded )
			{
				string Froms;
				string From;
				string Fromu;
				...
				} else
				{
					...
					SendAllChat( "Realm check enabled, allowed realms: "+m_Countries);
					m_Countries=m_Countries+" ??";
...
//				else // Unable to mute, more than one match found
			...
			if( Command == "holds" && !Payload.empty( ) && !m_GameLoading && !m_GameLoaded )
			{
				...
				while( !SS.eof( ) )
				{
					...
					else
					{
						...
						bool nsRootBanned = false;						

						for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
						{
							if (  (*i)->IsBannedName(HoldName)  )
							{
								nsRootBanned = true;
								CDBBan *nsban = (*i)->IsBannedName(HoldName);
								string nsAdmin = "";
								nsAdmin = nsban->GetAdmin();
								if ( IsRootAdmin(nsAdmin)  )
								{
									nsRootBanned = true;
								}
							}
							if (  (*i)->IsBannedIP(HoldName)  )
							{
								nsRootBanned = true;
								CDBBan *nsban = (*i)->IsBannedIP(HoldName);
								string nsAdmin = "";
								nsAdmin = nsban->GetAdmin();
								if ( IsRootAdmin(nsAdmin)  )
								{
						...
						if (nsRootBanned)
						{
							...
							{
								SendChat(player->GetPID(), "You have not enough rights to hold this player");
							}
							else
							{
	...
	// nobody can kick a rootadmin
			...
			if( (Command == "priv" || Command == "pri") && !Payload.empty( ) && !m_CountDownStarted && !m_SaveGame )
			{
				...
				if (GameName.empty())
				{
					GameName = m_GHost->IncGameNr ( m_GameName );
				}
				string s;
			...
			if( Command == "sendlan" && !Payload.empty( ) && !m_CountDownStarted )
			{
				...
				else
				{
					...
					if( m_SaveGame )
					{
						...
						MapHeight.push_back( 0 );
						m_GHost->m_UDPSocket->SendTo( IP, Port, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), MapWidth, MapHeight, m_GameName, "Nameless", GetTime( ) - m_CreationTime, "Save\\Multiplayer\\" + m_SaveGame->GetFileNameNoPath( ), m_SaveGame->GetMagicNumber( ), 12, 12, m_HostPort, m_HostCounter ) );
					}
					else
					{
						...
						MapGameType.push_back( 0 );
						MapGameType.push_back( 0 );
						m_GHost->m_UDPSocket->SendTo( IP, Port, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, "Nameless", GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), 12, 12, m_HostPort, m_HostCounter ) );
					}
				}
			}
...
//				if (((safe1 && safe2) || Payload.empty()) || RootAdminCheck)
			...
			/**/
			if( (Command == "nt" || Command == "nstime" || Command == "nstimer" || Command == "newtimer" ) && !Payload.empty( ) && !m_GameLoading && !m_GameLoaded )
			{
				m_nsCounter = UTIL_ToUInt32( Payload );
				SendAllChat( "Setting countdown timer to " + Payload);

			}/**/
			
			if( Command == "nml" || Command == "mutelast" || Command == "ml" )
			{
				SendAllChat( nsMuteLastReason );
			}/**/

			if( ( Command == "nstm" || Command == "nstalkingbot" || Command == "talk" || Command == "trololo" || Command == "troll" ) )
			{
				nstmKnownOnly = false;
				nstmReloadTXT = false;
				nstmAnswerAdmins = false;
				...
				if( nstmTalk )
				{
					SendAllChat( "Talking mode is ON");
					if ( !Payload.empty() )
					{
						for (int j = 0; j <= ((int) Payload.length()-1); j++ )
						{
							if (Payload.substr(j, 1) == "k" || Payload.substr(j, 1) == "K")
							{
								nstmKnownOnly = true;
							}
							if (Payload.substr(j, 1) == "r" || Payload.substr(j, 1) == "R")
							{
								nstmReloadTXT = true;
							}
							if (Payload.substr(j, 1) == "a" || Payload.substr(j, 1) == "A")
							{
								nstmAnswerAdmins = true;
							}
							if (Payload.substr(j, 1) == "t" || Payload.substr(j, 1) == "T")
							{
								m_GHost->nstmThreadedTalk = true;
							}							
							if (Payload.substr(j, 1) == "s" || Payload.substr(j, 1) == "S")
							{
								m_GHost->nstmThreadedTalk = false;
							}
						}
					}
				}
				...
				{
					SendAllChat( "Talking mode is OFF");
				}
			}/**/			
			
			if( ( Command == "testmode" ) && Payload.empty( ) && !m_GameLoading )
			{
				m_nmlsTestMode = !m_nmlsTestMode;
				if( nstmTalk )
				{
					SendAllChat( "testmode is ON");
				}
				else
				{
					SendAllChat( "testmode mode is OFF");
				}
			}/**/

			if( ( Command == "thread" ) && Payload.empty( ) && !m_GameLoading )
			{
				m_GHost->nstmThreadedTalk = !m_GHost->nstmThreadedTalk;
				if( m_GHost->nstmThreadedTalk )
				{
					SendAllChat( "ThreadedMode is ON");
				}
				else
				{
					SendAllChat( "ThreadedMode is OFF");
				}
			}/**/
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							if (isRootAdmin && !RootAdminCheck)//рут может свапать рута
							{
			...
			if( Command == "downloads" || Command == "dll" )
			{
	...
	*********************/
	...
	if( ( Command == "help" || Command == "h") && m_GHost->m_CurrentGame )
	{
		SendChat( player, "The nonadmin commands are:" );
		SendChat( player, "help(h), version(v), checkme, checkwarn(cw), checkban(cb), votekick, rmk, stats, banme(bm), countbans(cbs)" );
	}

	if( ( Command == "banme" || Command == "bm" ) )
	{
		string Victim = User;
		string Reason = "has banned himself!";

		if (IsAdmin(Victim) || IsRootAdmin(Victim))
		{
			SendChat( player, "You can't ban an admin!" );
			return HideCommand;
		}

		if (IsSafe(Victim) && m_GHost->m_SafelistedBanImmunity)
		{
			SendChat( player, "You can't ban a safelisted player!" );
			return HideCommand;
		}
		...
		{
			if( (*i)->GetServer( ) == GetCreatorServer( ) )
				if ((*i)->IsBannedName(Victim))
					isBanned = true;
		}
		
		if( isBanned )
		{
			SendAllChat( "Player [" + User + "] is already banned" );
		}
		else
		{
			uint32_t nsBanTime = m_GHost->m_BanLastTime;
			transform( Victim.begin( ), Victim.end( ), Victim.begin( ), (int(*)(int))tolower );
			m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, Victim, Victim, m_GameName, User, Reason, nsBanTime, 0 ) ) );
			CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + User + "] " + Reason );
			SendAllChat( "Player [" + User + "] " + Reason );
		}
	}
	...
	if( (Command == "countbans" || Command == "cbs") && Payload.empty() )
	{
		...
		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			uint32_t WC = m_GHost->m_DB->BanCount( (*i)->GetServer( ) );
			if( WC == 0 )
				SendAllChat( m_GHost->m_Language->ThereAreNoBannedUsers( (*i)->GetServer( ) ) );
			else if( WC == 1 )
				SendAllChat( m_GHost->m_Language->ThereIsBannedUser( (*i)->GetServer( ) ));
			else
				SendAllChat( m_GHost->m_Language->ThereAreBannedUsers( (*i)->GetServer( ), UTIL_ToString( WC ) ) );
			WarnCount += WC;
		}
		SendAllChat( "Total bans count: " + UTIL_ToString( WarnCount ) );
	}
	...
	if( (Command == "checkban" || Command == "cb") && !m_GHost->m_BNETs.empty( ) )
	{
		if ( Payload.empty( ) )
		{
			Payload = User;
	...
	if( ( Command == "checkwarn" || Command == "checkwarns" || Command == "cw" ) && !m_GHost->m_BNETs.empty( ) )
	{
		if ( Payload.empty( ) )
		{
			Payload = User;
		...
		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			...
			{
				SendAllChat("User [" + Payload + "] is not warned on server [" + (*i)->GetServer( ) + "]" );
			}
		}
	}
	...
	if( ( Command == "crash" ) )
	{
		//string tochat = m_GHost->m_DB->ThreadedGamePlayerSummaryCheck( User );
		//SendAllChat ( tochat );
...
void CGame :: WarnPlayer( CDBBan *LastMatch, string Reason, string User)
{
	...
	string strchr = LastMatch->GetIP( );

	if ( strchr.length() <2)
	{
		m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetServer(), LastMatch->GetName(), LastMatch->GetName(), m_GameName, User, Reason, m_GHost->m_WarnTimeOfWarnedPlayer, 1 ));
	}
	else
	{
		m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetServer(), LastMatch->GetName(), LastMatch->GetIP( ), m_GameName, User, Reason, m_GHost->m_WarnTimeOfWarnedPlayer, 1 ));
	...
	if(WarnCount >= m_GHost->m_BanTheWarnedPlayerQuota)
	{
		...
		if (!isAdmin)
		{
			string lenip = LastMatch->GetIP();
			if (lenip.length() < 2)
			{
			m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetServer( ), LastMatch->GetName( ),  LastMatch->GetName( ), m_GameName, User, "Reached the warn quota", m_GHost->m_BanTimeOfWarnedPlayer, 0 ));
			}
			else
			{
		...
		for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
		{
			if ((*j)->GetServer() == m_Server)
			{
				...
				string str2l = LastMatch->GetIP();

				if (str2l.length() < 2 )
				{
					(*j)->AddBan(LastMatch->GetName(), LastMatch->GetName(), LastMatch->GetGameName(), LastMatch->GetAdmin(), Reason, sDate);				
				}
				else
				{
					(*j)->AddBan(LastMatch->GetName(), LastMatch->GetIP(), LastMatch->GetGameName(), LastMatch->GetAdmin(), Reason, sDate);
...
void CGame :: WarnPlayer( string Victim, string Reason, string User)
{
	...
	if( Matches <= 1 )
	{
		...
		if (Matches == 1)
		{
			string strlenip = LastMatch->GetExternalIPString( );
			if ( ( LastMatch->GetExternalIPString( ) == "127.0.0.1" ) || (LastMatch->GetExternalIPString( ) == player->GetExternalIPString() ) || (strlenip.length() < 2 ) )
			{
				m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetJoinedRealm( ), LastMatch->GetName( ), LastMatch->GetName( ), m_GameName, User, Reason, m_GHost->m_WarnTimeOfWarnedPlayer, 1 ) ) );
			}
			else
			{
			...
			m_PairedBanAdds.push_back( PairedBanAdd( User, m_GHost->m_DB->ThreadedBanAdd( m_Server, Victim, Victim, m_GameName, User, Reason, m_GHost->m_WarnTimeOfWarnedPlayer, 1 ) ) );
		...
		CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + BanPlayer + "] was warned by player [" + User + "] Warncount " + UTIL_ToString(WarnCount));
		...
		if(WarnCount >= m_GHost->m_BanTheWarnedPlayerQuota)
		{
			...
			if (Matches==1)
			{
				string str1 = LastMatch->GetExternalIPString( );
				if ( str1.length() < 2)
				{
					m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetSpoofedRealm( ), BanPlayer, BanPlayer, m_GameName, User, "Reached the warn quota", m_GHost->m_BanTimeOfWarnedPlayer, 0 ));
				}
				else
				{
				...
				m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( m_Server, BanPlayer, BanPlayer, m_GameName, User, "Reached the warn quota", m_GHost->m_BanTimeOfWarnedPlayer, 0 ));
			...
			for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
			{
				if ((*j)->GetServer() == m_Server)
				{
					...
					if (Matches==1)
					{
						string srtdve = LastMatch->GetExternalIPString();
						if (srtdve.length() < 2)
						{
							(*j)->AddBan(LastMatch->GetName(), LastMatch->GetName(), m_GameName, User, Reason, sDate);
						}
						else
						{
							(*j)->AddBan(LastMatch->GetName(), LastMatch->GetExternalIPString(), m_GameName, User, Reason, sDate);
						...
						(*j)->AddBan(BanPlayer, BanPlayer, m_GameName, User, Reason, sDate);
				}
			}
		}

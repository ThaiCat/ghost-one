...
{
	string RusLower = "—ë–π—Ü—É–∫–µ–Ω–≥—à—â–∑—Ö—ä—Ñ—ã–≤–∞–ø—Ä–æ–ª–¥–∂—ç—è—Å—á–º–∏—Ç—å–±—é";
	string RusUpper = "–Å–ô–¶–£–ö–ï–ù–ì–®–©–ó–•–™–§–´–í–ê–ü–†–û–õ–î–ñ–≠–Ø–°–ß–ú–ò–¢–¨–ë–Æ";
...
{
	string RusLower = "—ë–ª—Å—å–π—Ü—É–∫–µ–Ω–≥—à—â–∑—Ö—ä—Ñ—ã–≤–∞–ø—Ä–æ–¥–∂—ç—è—á–º–∏—Ç–±—é";
	string RusUpper = "–Å–õ–°–¨–ô–¶–£–ö–ï–ù–ì–®–©–ó–•–™–§–´–í–ê–ü–†–û–î–ñ–≠–Ø–ß–ú–ò–¢–ë–Æ";
	string EnLower = "qwertyuiopasdfghjklzxcvbnm";
	string EnUpper = "QWERTYUIOPASDFGHJKLZXCVBNM";
	string result = "";
	bool RusDetected = false;
...
/*
	...
	//if ( !nstmTalkReaded || nstmReloadTXT )
	{
		...
		ifstream in;
		in.open( "talk.txt" );
		int Switcher = 1; //ÚÛÚ Í‡Í‡ˇ-ÚÓ ÌÂ‚Â‰ÓÏ‡ˇ ıÛÈÌˇ Ú‚ÓËÚÒˇ Ò ÔËÒ‚ÓÂÌËÂÏ!
		PhrasePairCount = 0;
		if( !in.fail() )
		{
			string Line;
			while( !in.eof() )
			{
				...
				if ( Line == "#" )
				{
					...
					PhraseQ[PhrasePairCount].clear();////‰ÓÎÊÌÓ ÔÓÍ‡ÚËÚ¸
	...
	for (int i = 0; i <= PhrasePairCount; i++) /// Ì‡‚ÂÌÓ ÚÛÚ
	{		
...
void CBaseGame :: nsBotAnswerThread (void* fwd_data_t)
{
	...
	struct nsFunctions
	{
		...
		{
			string RusLower = "—Å—ë–ª—å–π—Ü—É–∫–µ–Ω–≥—à—â–∑—Ö—ä—Ñ—ã–≤–∞–ø—Ä–æ–¥–∂—ç—è—á–º–∏—Ç–±—é";
			string RusUpper = "–°–Å–õ–¨–ô–¶–£–ö–ï–ù–ì–®–©–ó–•–™–§–´–í–ê–ü–†–û–î–ñ–≠–Ø–ß–ú–ò–¢–ë–Æ";
		...
		string nstransliterate ( string WhatToParse )
		{
			...
			//string EngLower = "Ò_∏_Î_¸_È_ˆ_Û_Í_Â_Ì_„_¯_˘_Á_ı_˙_Ù_˚_‚_‡_Ô__Ó_‰_Ê_˝_ˇ_˜_Ï_Ë_Ú_·_˛_"; //ÌÂ ‡·ÓÚ‡ÂÚ
			string WhatToAdd;
			int CLength = (int) RusUpper.length()-1;
			int WLength = (int) WhatToParse.length();
		...
		string nsanalize ( string WhatToAnalize )
		{
			...
			{
				Contain = false;
				for(int j = 0; j < (int) AnalizedString.length(); j++) //ÏÓÊÂÚ ÏÂÌˇÚ¸Òˇ!
				{
	...
	if ( nstmTalkInThread )
	{
		if ( nsQuestion[0] != trigger )
		{
			...
			if ( !nstmTalkReadedInThread || nstmReloadTXTInThread || true )
			{
				...
				int SwitcherInThread = 1;//ÚÛÚ Í‡Í‡ˇ-ÚÓ ÌÂ‚Â‰ÓÏ‡ˇ ıÛÈÌˇ Ú‚ÓËÚÒˇ Ò ÔËÒ‚ÓÂÌËÂÏ!
				ifstream in;
				in.open( "talk.txt" );
				if( !in.fail() )
				{
					while( !in.eof() )
					{
						...
						if ( Line == "#" )
						{
							...
							SwitcherInThread = 1;
							PhraseQInThread[PhrasePairCountInThread].clear();//‰ÓÎÊÌÓ ÔÓÍ‡ÚËÚ¸
			...
			for (int i = 1; i <= PhrasePairCountInThread; i++)//ıÛËÚ‡ Í‡Í‡ˇ-ÚÓ ‰Ó·‡‚ÎÂÌÓ ËÁ-Á‡ Â¯ÂÚÓÍ ‚ Ì‡˜‡ÎÂ ÚÓÎÍ‡ (i=1)
			{
			...
			AnswerPhrase = nsf.nsreplace ( "$PARTAFTERKEYWORD$", nsf.nsafter( nsKeyWord, nsQuestion), AnswerPhrase );/////////‚ÓÁÏÓÊÂÌ ·‡„ ÂÒÎË ÌÂ Ô‡ÒËÚ¸
...
void CBaseGame :: nsBotAnswer( string nsQuestion, string nsPlayer, bool admin, char trigger)
{
	...
	if ( nstmTalk )
	{
		if ( nsQuestion[0] != trigger )
		{
			...
			for (int i = 1; i <= PhrasePairCount; i++)//ıÛËÚ‡ Í‡Í‡ˇ-ÚÓ ‰Ó·‡‚ÎÂÌÓ ËÁ-Á‡ Â¯ÂÚÓÍ ‚ Ì‡˜‡ÎÂ ÚÓÎÍ‡ (i=1)
			{
...
{
	BYTEARRAY data = m_Protocol->SEND_W3GS_CHAT_FROM_HOST( GetHostPID( ), GetPIDs( ), 32, UTIL_CreateByteArray( (uint32_t)0, false ), "ÚÂÒÚ" );
	for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); ++i )
    {
		(*i)->Send( data );
	}
...
void CBaseGame :: nsFloodMute( string nsPlayerIDS, string nsMessage, bool nsIsRootAdmin, bool nsIsAdmin)
{
			...
			//string Rus = "—ë–π—Ü—É–∫–µ–Ω–≥—à—â–∑—Ö—ä—Ñ—ã–≤–∞–ø—Ä–æ–ª–¥–∂—ç—è—á—Å–º–∏—Ç—å–±—é–Å–ô–¶–£–ö–ï–ù–ì–®–©–ó–•–™–§–´–í–ê–ü–†–û–õ–î–ñ–≠–Ø–ß–°–ú–ò–¢–¨–ë–Æ";
			//bool Engl;
			string TUPOST = " _";
			string OnlyDigitsAN = "";

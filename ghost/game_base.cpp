...
CBaseGame :: CBaseGame( CGHost *nGHost, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, string nGameName, string nOwnerName, string nCreatorName, string nCreatorServer )
{
	...
	m_nsCounter = 0;
	nstmTalk = false;
	m_nmlsTestMode = false;
	for (int jjj = 0; jjj <= 11; jjj++)
	{

	FloodMsg[jjj][0] = "X yu";
	FloodMsg[jjj][1] = "PiZ da";
	FloodMsg[jjj][2] = "Dji GurDa";
	
	FloodMsgAN[jjj][0] = "Xy u";
	FloodMsgAN[jjj][1] = "PiZ da";
	FloodMsgAN[jjj][2] = "DjiG urDa";

	MsgInfortativity[jjj][2] = 1;
	MsgInfortativity[jjj][1] = 1;
	MsgInfortativity[jjj][0] = 1;

	MsgTypingSpeed[jjj][2] = 1;
	MsgTypingSpeed[jjj][1] = 1;	
	MsgTypingSpeed[jjj][0] = 1;

	}

	nstmTalkReaded = false;
	...
	if( GetTime( ) - m_LastPingTime >= 5 )
	{
		...
		if( !m_CountDownStarted )
		{
			...
			if( m_SaveGame )
			{
				...
				{
					m_GHost->m_UDPSocket->Broadcast( 6112, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), MapWidth, MapHeight, m_GameName, "Nameless", GetTime( ) - m_CreationTime, "Save\\Multiplayer\\" + m_SaveGame->GetFileNameNoPath( ), m_SaveGame->GetMagicNumber( ), slotstotal, slotsopen, m_HostPort, FixedHostCounter ) );
					m_GHost->m_UDPSocket->SendTo("127.0.0.1", 6112, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), MapWidth, MapHeight, m_GameName, "Nameless", GetTime( ) - m_CreationTime, "Save\\Multiplayer\\" + m_SaveGame->GetFileNameNoPath( ), m_SaveGame->GetMagicNumber( ), slotstotal, slotsopen, m_HostPort, FixedHostCounter ) );
				}
//				if (m_GarenaOnly)
				{
				...
				m_GHost->UDPChatSend(m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), MapWidth, MapHeight, m_GameName, "Nameless", GetTime( ) - m_CreationTime, "Save\\Multiplayer\\" + m_SaveGame->GetFileNameNoPath( ), m_SaveGame->GetMagicNumber( ), slotstotal, slotsopen, m_HostPort, FixedHostCounter ) );
			}
			else
			{
				...
				if (m_GHost->m_broadcastinlan)
				{
					m_GHost->m_UDPSocket->Broadcast( 6112, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, "Nameless", GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), slotstotal, slotsopen, m_HostPort, FixedHostCounter ) );
					m_GHost->m_UDPSocket->SendTo( "127.0.0.1", 6112, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, "Nameless", GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), slotstotal, slotsopen, m_HostPort, FixedHostCounter ) );
				}
//				if (m_GarenaOnly)
				{
...
//					m_GHost->m_UDPSocket->SendTo( "192.168.1.2", 6112, m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, "Varlock", GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), 12, 12, m_HostPort, FixedHostCounter ) );
				...
				m_GHost->UDPChatSend(m_Protocol->SEND_W3GS_GAMEINFO( m_GHost->m_TFT, m_GHost->m_LANWar3Version, MapGameType, m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, "Nameless", GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), 12, 12, m_HostPort, FixedHostCounter ));
			}
		}
...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	if (potential->IsLAN() || potential->GetGarenaUser( ) != NULL )
	{
		bool nsisBanned = false;
		string nsAdmin = "";
		string nsReason = "";

		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			if ( (*i)->IsBannedIP(joinPlayer->GetName( )) )
			{
				nsisBanned = true;
				CDBBan *nsban = (*i)->IsBannedIP(joinPlayer->GetName( ));
				nsAdmin = nsban->GetAdmin();
				nsReason = nsban->GetReason();
			}
		}

		if (nsisBanned)
		{
			CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + joinPlayer->GetName( ) + "|" + potential->GetExternalIPString( ) + "] is trying to join the game but is banned by name (LAN)" );

			if (m_GHost->m_Verbose)
			{
				string nsBan = joinPlayer->GetName() + " is trying to join the game but is banned by " + nsAdmin;
				string nsBReason = nsBan + ": "+nsReason;
				
				if ( nsReason == "" )
				{
					SendAllChat( nsBan + " (LAN)" );
				}
				else
				{
					if (nsBReason.length()<250 && !m_GHost->m_TwoLinesBanAnnouncement)
					{
						SendAllChat( nsBReason + " (LAN)" );
					}
					else
					{
						SendAllChat( nsBan );
						SendAllChat( "Ban reason: " + nsReason + " (LAN)" );
					}
				}
			}
	...
	if (!Reserved)
	if (!m_ScoreCheckChecked)
	if (!potential->IsLAN()) //added by me
	{
		bool nsisBanned = false;
		string nsAdmin = "";
		string nsReason = "";
		...
		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			if ( (*i)->IsBannedName( joinPlayer->GetName( ) ) )
			{
				nsisBanned = true;
				CDBBan *nsban = (*i)->IsBannedName( joinPlayer->GetName( ) );
				nsAdmin = nsban->GetAdmin();
				nsReason = nsban->GetReason();
			}
		}
		...
		if (nsisBanned)
		{
			...
			if (m_GHost->m_Verbose)
			{
				...
				else
				{
					if (nsBReason.length()<250 && !m_GHost->m_TwoLinesBanAnnouncement)
					{
						SendAllChat( nsBReason );
					}
					else
					{
						SendAllChat( nsBan );
						SendAllChat( "Ban reason: " + nsReason );
					}
				}
			}
			vector<CGameSlot> Slots = m_Map->GetSlots( );
	...
	if (m_GHost->m_IPBanning!=0)
	{
		...
		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			if ( (*i)->IsBannedIP(potential->GetExternalIPString( ) ) )
			{
				nsisBanned = true;
				CDBBan *nsban = (*i)->IsBannedIP(potential->GetExternalIPString( ) );
				nsName = nsban->GetName();
				nsAdmin = nsban->GetAdmin();
				nsReason = nsban->GetReason();
			}
		}
		if (nsisBanned)
		{
			...
			if (m_GHost->m_Verbose)
			{
				string nsBan = m_GHost->m_Language->TryingToJoinTheGameButBanned( joinPlayer->GetName() + " (IP of  " + nsName + ")", nsAdmin );
				string nsBReason = nsBan + ": "+nsReason;

				if ( ( m_GHost->m_IPBanning == 1 ) || ( m_GHost->m_IPBanning == 2 && IsRootAdmin( nsAdmin ) ) )///////////////////
				{
					if ( nsReason == "" )
					{
						SendAllChat( nsBan );
    ...
    if (m_GHost->nsEnableStayPercentageCheckNew)
	{
		...
		if(SinglePairNS.second->GetReady())
		{
			...
			if( GamePlayerSummaryNS )
			{
				...
				{
					nsRequaredPercentageStay = m_GHost->nsStayRequaredPercentage1moreNew;
				}
				else
				{
					if ( nsGamesPlayedCount >= 2 && nsGamesPlayedCount < 4 )
					{
						nsRequaredPercentageStay = m_GHost->nsStayRequaredPercentage2moreNew;
					}
					else
					{
						if( nsGamesPlayedCount >= 4 )
						{
							nsRequaredPercentageStay = m_GHost->nsStayRequaredPercentage4moreNew;
						}
					}

				}
				if (nsAveragePercentageStay > nsRequaredPercentageStay)
				{
				}
				else
				{
			...
			{
				if (m_GHost->nsAllowUnknownStayPercentagePlayersNew)
				{
				}
				else
				{
					if (m_GHost->m_Verbose)
					{
						SendAllChat( "Player [" + joinPlayer->GetName()+ "] never played here and new players are disabled." );
					}
					//кик
					// let banned players "join" the game with an arbitrary PID then immediately close the connection
					// this causes them to be kicked back to the chat channel on battle.net
	...
	*/
	...
	if (m_GarenaOnly)
	{
		...
		// kick if not garena, admin, rootadmin, reserved
...
/**/
	...
	for (int i = 1; i < 32; i++)
	{
		if ( ( m_GHost->ns_server_server[i] == JoinedRealm ) && (m_GHost->ns_server_server[i] != string() ) && (m_GHost->ns_server_alias[i] != string() ) )
		{
			sendserver = m_GHost->ns_server_alias[i];
			if ( sendserver.length() > 0 )
			{
				break;
			}
		}
	...
	{
		if (Player->GetGarenaUser() != NULL)
		{
			PlayerJoinedFromMsg += "Garena].";
		}
		else
		{
			PlayerJoinedFromMsg += "LAN].";
		}
	}
	else
	{
...
// kick if not garena, admin, rootadmin, reserver
	...
	CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + Player->GetName( ) + "] joined the from [" + ( JoinedRealm == string( ) ? "LAN" : JoinedRealm ) + "]" );
	/* from <" + ( JoinedRealm == string( ) ? "LAN" : JoinedRealm ) + ">" );*/
...
string CBaseGame :: nsRuToLower( string Question )
{
	string RusLower = "С‘Р№С†СѓРєРµРЅРіС€С‰Р·С…СЉС„С‹РІР°РїСЂРѕР»РґР¶СЌСЏСЃС‡РјРёС‚СЊР±СЋ";
	string RusUpper = "РЃР™Р¦РЈРљР•РќР“РЁР©Р—РҐРЄР¤Р«Р’РђРџР РћР›Р”Р–Р­РЇРЎР§РњРРўР¬Р‘Р®";
	string EnLower = "qwertyuiopasdfghjklzxcvbnm";
	string EnUpper = "QWERTYUIOPASDFGHJKLZXCVBNM";
	string result = "";
	bool RusDetected = false;
	bool EnDetected = false;
	int jj;

	for (int i = 0; i <= ((int) Question.length()-1); i++ )
	{
		RusDetected = false;
		EnDetected = false;
		for (int j = 0; j <= ((int) RusUpper.length()-2); j++ )
		{
			jj = j;
			if ( Question.substr( i, 2 ) == RusUpper.substr( j, 2 ) )
			{
				RusDetected = true;
				break;
			}
		...
		if ( RusDetected )
		{
			result += RusLower.substr( jj, 2 );
			i++;
		}
		else
		{
			for (int j = 0; j <= ((int) EnUpper.length()-1); j++ )
			{
				if ( Question.substr( i, 1 ) == EnUpper.substr( j, 1 ) )
				{
					EnDetected = true;
					result += EnLower.substr( j, 1 );
					break;
				}
			}
			if ( !EnDetected )
			{
				result += Question.substr( i, 1 );
			}
		}
	}
	return result;
...
bool CBaseGame :: nsFindKeyWord ( string KeyWord, string Question )
{
	string LowerKeyWord = nsRuToLower ( KeyWord );
	string LowerQuestion = nsRuToLower ( Question );
	bool result = false;
	for (int i = 0; i <= ( (int) ( LowerQuestion.length() - LowerKeyWord.length() ) ); i++)
	{
		if ( LowerQuestion.substr( i, LowerKeyWord.length() ) == LowerKeyWord )
		{
			result = true;
			break;
		}
...
void CBaseGame :: nsGameBaseConsolePrint (string nsMessage)
{
	CONSOLE_Print( nsMessage );
}
/*
string nsRuToLowerInThread( string Question )
{
	string RusLower = "С‘Р»СЃСЊР№С†СѓРєРµРЅРіС€С‰Р·С…СЉС„С‹РІР°РїСЂРѕРґР¶СЌСЏС‡РјРёС‚Р±СЋ";
	string RusUpper = "РЃР›РЎР¬Р™Р¦РЈРљР•РќР“РЁР©Р—РҐРЄР¤Р«Р’РђРџР РћР”Р–Р­РЇР§РњРРўР‘Р®";
	string EnLower = "qwertyuiopasdfghjklzxcvbnm";
	string EnUpper = "QWERTYUIOPASDFGHJKLZXCVBNM";
	string result = "";
	bool RusDetected = false;
...
/*
bool nsFindKeyWordInThread ( string KeyWord, string Question )
{
	bool result = false;
	for (int i = 0; i <= ( (int) ( Question.length() - KeyWord.length() ) ); i++)
	{
		if ( Question.substr( i, KeyWord.length() ) == nsRuToLowerInThread ( KeyWord ) )
		{
			result = true;
			break;
		}
...
*/
void CBaseGame :: nsBotAnswerThreadPrepare( const char *question, const char *nsplayer, bool admin, char trigger )
{
	//it is very important to wait at the end of this function
	//if it returns value before thread function get the variables
	//it can cause really bad shit
	//HANDLE handle;
	Data data;
	data.handle = CreateEvent(NULL, false, false, NULL);
	data.handle2 = CreateEvent(NULL, false, false, NULL);
	data.question = question;
	data.player = nsplayer;
	data.gamename = m_GameName.c_str();
	data.trigger = trigger;
	data.admin = admin;
	data.TimeToWaitMax = m_GHost->nstmTimeMaxToAdd;
	data.TimeToWaitMin = m_GHost->nstmTimeMinToAdd;
	data.ToSleep = m_GHost->nstmAddTime;
	data.pthis = this;
	if (data.pthis == NULL)
	{
		CONSOLE_Print( "[TEST] data.pthis NULL ERROR" );
	}
/*
	CONSOLE_Print( "[TEST] start loading" );
	//if ( !nstmTalkReaded || nstmReloadTXT )
	{
		for (int i = 0; i < 999; i++)
		{
			PhraseQCount[i] = 0;
		...
		PhraseUCount = 0;
		CONSOLE_Print( "[TEST] start loading 1" );
		ifstream in;
		in.open( "talk.txt" );
		int Switcher = 1; //тут какая-то неведомая хуйня творится с присвоением!
		PhrasePairCount = 0;
		if( !in.fail() )
		{
			...
			while( !in.eof() )
			{
				...
				if ( Line == "#" )
				{
					...
					PhraseQ[PhrasePairCount].clear();////должно прокатить
					PhraseQCount[PhrasePairCount] = 0;
				}
				if ( Line == "##" )
				{
					Switcher = 2;

					PhraseA[PhrasePairCount].clear();
					PhraseACount[PhrasePairCount] = 0;
				}
				if ( Line == "###" )
				{
					Switcher = 3;
				...
				if ( ( Line != "#" ) && ( Line != "##" ) && ( Line != "###" ) )
				{
					if ( Switcher == 1 )
					{						
						PhraseQ[PhrasePairCount].insert( Line );
						PhraseQCount[PhrasePairCount]++;
					}
					if ( Switcher == 2 )
					{
						PhraseA[PhrasePairCount].insert( Line );
						PhraseACount[PhrasePairCount]++;
					}
					if ( Switcher == 3 )
					{
						PhraseU.insert( Line );
	...
	CONSOLE_Print( "[TEST] finished loading" );

	data.PhraseUCount = PhraseUCount;
	data.PhraseU = PhraseU;
	data.PhrasePairCount = PhrasePairCount;
	CONSOLE_Print( "[TEST] finished loading 2" );
	memcpy(data.PhraseA, PhraseA, sizeof(PhraseA));
	memcpy(data.PhraseQ, PhraseQ, sizeof(PhraseQ));

	for (int i = 0; i <= PhrasePairCount; i++) /// наверно тут
	{		
		data.PhraseACount[i] = PhraseACount[i];
		data.PhraseQCount[i] = PhraseQCount[i];
		//data.PhraseA[i] = PhraseA[i];
		//data.PhraseQ[i] = PhraseQ[i];
		//data.RandPhraseANumber[i] = RandPhraseANumber[i];
	}

	CONSOLE_Print( "[TEST] finished loading 3" );*/;
	//handle = 

	nsBotAnswerThreadLaunch (&data);
	//_beginthread( nsBotAnswerThread, 0, &data );
	//_beginthread( Bounce, 0, (void *) (ch++)  );
	//boost::thread TalkThread(nsBotAnswerThread, &data);
	//boost::thread PrintThread( CONSOLE_PrintThread, boost::ref( message ) );
	//CONSOLE_Print( "[TEST] finished loading 4" );
	//CONSOLE_Print( "[TEST] 1" );

	WaitForSingleObject(data.handle, INFINITE);
	//CloseHandle(data.handle);
	//CloseHandle(data.handle2);
	//free (&data);
	//ResetEvent(data.handle);
	//delete (&data);
	//CONSOLE_Print( "[TEST] 2" );
	//SetEvent(data.handle);//2й раз
	//2й раз


	
	//CONSOLE_Print( "[TEST] Preparator END" );

	return;	
}

void CBaseGame :: nsBotAnswerThreadLaunch (struct Data *data)
{
	struct Data *fwd_data_l = (struct Data *)malloc(sizeof(struct Data));

	if (!fwd_data_l)
	{
		CONSOLE_Print( "[TEST] memory allocation fail" );
		return;// 0;
	}

	*fwd_data_l = *data;

	HANDLE handle =(HANDLE)_beginthread( nsBotAnswerThread, 4096, fwd_data_l );
	//HANDLE handle =(HANDLE)_beginthread( nsBotAnswerThread, 0, data );
	if (!handle)
	{
		CONSOLE_Print( "[TEST] thread run fail" );
	}
	//CONSOLE_Print( "[TEST] 3" );

	//free(fwd_data_l);

	//return;// handle ? handle : (free(fwd_data_l), 0);

	//WaitForSingleObject(data->handle2, INFINITE);
	//free (fwd_data_l);
	//CONSOLE_Print( "[TEST] 4" );


	//WaitForSingleObject( (Data *)data.handle ,INFINITE);

	//CONSOLE_Print( "[TEST] Launcher END" );

	return;
}
/**/
void CBaseGame :: nsBotAnswerThread (void* fwd_data_t)
{
	struct Data data = *(struct Data *)fwd_data_t;
	free (fwd_data_t);
	...
	TimeToWaitMax = data.TimeToWaitMax;
	TimeToWaitMin = data.TimeToWaitMin;
	ToSleep = data.ToSleep;
	string nsQuestion = question;
	string nsPlayer = player;
	string nsGameName = gamename;

	bool nstmAnswerAdminsInThread = data.pthis->nstmAnswerAdmins;
	bool nstmTalkInThread = data.pthis->nstmTalk;
	bool nstmReloadTXTInThread = data.pthis->nstmReloadTXT;
	bool nstmTalkReadedInThread = data.pthis->nstmTalkReaded;
	bool nstmKnownOnlyInThread = data.pthis->nstmKnownOnly;


	vector <set <string>> PhraseQ;
	vector <set <string>> PhraseA;
	vector <uint32_t> CountPhraseQ;
	vector <uint32_t> CountPhraseA;
	vector <uint32_t> RandomPhraseQ;
	vector <uint32_t> RandomPhraseA;
	//vector<int> myvector;

	//questions/keywords
	set <string> PhraseQInThread[999];
	uint32_t RandPhraseQNumberInThread[999];
	uint32_t PhraseQCountInThread[999];

	//answers
	set <string> PhraseAInThread[999];
	uint32_t RandPhraseANumberInThread[999];
	uint32_t PhraseACountInThread[999];

	//answers for unknown keywords
	set <string> PhraseUInThread;
	uint32_t RandPhraseUNumberInThread;
	uint32_t PhraseUCountInThread;

	int PhrasePairCountInThread;

	uint32_t begintime = GetTicks();
	srand ( GetTicks() );
	
	int nsFullAnswerTime = rand()%(TimeToWaitMax - TimeToWaitMin) + TimeToWaitMin;

	SetEvent(data.handle);//now nsBotAnswerThreadPrepare can return value

	struct nsFunctions
	{
		string ns2lower( string Question )
		{
			string RusLower = "СЃС‘Р»СЊР№С†СѓРєРµРЅРіС€С‰Р·С…СЉС„С‹РІР°РїСЂРѕРґР¶СЌСЏС‡РјРёС‚Р±СЋ";
			string RusUpper = "РЎРЃР›Р¬Р™Р¦РЈРљР•РќР“РЁР©Р—РҐРЄР¤Р«Р’РђРџР РћР”Р–Р­РЇР§РњРРўР‘Р®";
			string EngLower = "qwertyuiopasdfghjklzxcvbnm";
			string EngUpper = "QWERTYUIOPASDFGHJKLZXCVBNM";
			string nsResult = "";
			bool RusDetected = false;
			bool EngDetected = false;
			int jj;
			...
			for (int i = 0; i <= ((int) Question.length()-1); i++ )
			{
				...
				if ( RusDetected )
				{
					nsResult += RusLower.substr( jj, 2 );
					i++;
				}
				else
				{
					for (int j = 0; j <= ((int) EngUpper.length()-1); j++ )
					{
						if ( Question.substr( i, 1 ) == EngUpper.substr( j, 1 ) )
						{
							EngDetected = true;
							nsResult += EngLower.substr( j, 1 );
							break;
						}
					}
					if ( !EngDetected )
					{
						nsResult += Question.substr( i, 1 );
					}
				}
			}
			return nsResult;
		}
		bool nsfound ( string KeyWord, string Question )
		{
			string LowerKeyWord = ( KeyWord );
			string LowerQuestion = ( Question );
			bool result = false;   
			for (int i = 0; i <= ( (int) ( LowerQuestion.length() - LowerKeyWord.length() ) ); i++)
			{
				if ( LowerQuestion.substr( i, LowerKeyWord.length() ) == LowerKeyWord )
		...
		bool nsfoundl ( string KeyWord, string Question )
		{
			string LowerKeyWord = ns2lower ( KeyWord );
			string LowerQuestion = ns2lower ( Question );
			bool result = false;   
			for (int i = 0; i <= ( (int) ( LowerQuestion.length() - LowerKeyWord.length() ) ); i++)
			{
				if ( LowerQuestion.substr( i, LowerKeyWord.length() ) == LowerKeyWord )
		...
		int nsfoundi ( string KeyWord, string Question )
		{
			string LowerKeyWord = ( KeyWord );
			string LowerQuestion = ( Question );
			int result = -1;   
		...
		int nsfoundil ( string KeyWord, string Question )
		{
			string LowerKeyWord = ns2lower ( KeyWord );
			string LowerQuestion = ns2lower ( Question );
			int result = -1;   
		...
		string nsreplace ( string ReplaceWhat, string ReplaceToWhat, string ReplaceWhere )
		{
			string result = ReplaceWhere;
			int Position =  nsfoundi ( ReplaceWhat, ReplaceWhere );
			if ( Position != -1 )
			{
				string part1 = ReplaceWhere.substr( 0, Position );
				string part2 = ReplaceWhere.substr( Position + ReplaceWhat.length(), ReplaceWhere.length() - (Position + ReplaceWhat.length()) );
				result = part1 + ReplaceToWhat + part2;
			}
			return result;
		}
		string nsbefore ( string KeyWord, string Message )
		{
			string result = "";
			int Position =  nsfoundi ( KeyWord, Message );
			if ( Position != -1 )
			{
		...
		string nsafter ( string KeyWord, string Message )
		{
			string result = "";
			int Position =  nsfoundi ( KeyWord, Message );			
			if ( Position != -1 )
			{
				result = Message.substr( Position + KeyWord.length(), Message.length() - (Position + KeyWord.length()) );
			}
			return result;
		}
		string nsnospace ( string ReplaceWhere )
		{
			string result = ReplaceWhere;
			for (int i = 0; i < (int) result.length(); i++)
			{
				result = nsreplace ( "  ", " ", result );
			}
			if ( result.length() > 1 )
		...
		string nsparse ( string WhatToParse )
		{
			string ParsedString = "";
			string RusLower = "СЃС‘Р»СЊР№С†СѓРєРµРЅРіС€С‰Р·С…СЉС„С‹РІР°РїСЂРѕРґР¶СЌСЏС‡РјРёС‚Р±СЋ";
			string RusUpper = "РЎРЃР›Р¬Р™Р¦РЈРљР•РќР“РЁР©Р—РҐРЄР¤Р«Р’РђРџР РћР”Р–Р­РЇР§РњРРўР‘Р®";
			string WhatToAdd;
			int CLength = (int) RusUpper.length()-1;
			int WLength = (int) WhatToParse.length();
			...
			for (int i = 0; i < WLength; i++ )
			{
				...
				{
					if ( 
						( WhatToParse.substr( i, 2 ) == RusUpper.substr( j, 2 ) ) || 
						( WhatToParse.substr( i, 2 ) == RusLower.substr( j, 2 ) )
						)
					{
						WhatToAdd = WhatToParse.substr( i, 2 );
						RusDetected = true;
						break;
					}
				}
				if ( RusDetected )
				{
					...
					{
						if( ParsedString.substr( ParsedString.length() - 2, 2 ) != WhatToAdd )
						{
							ParsedString += WhatToAdd;
						}
					}
					else
					{
				...
				else
				{
					...
					{
						if( ParsedString.substr( ParsedString.length() - 1, 1 ) !=  WhatToParse.substr( i, 1 ) )
						{
							ParsedString += WhatToParse.substr( i, 1 );
						}
					}
					else
					{
		...
		string nstransliterate ( string WhatToParse )
		{
			string ParsedString = "";
			string RusLower = "СЃС‘Р»СЊР№С†СѓРєРµРЅРіС€С‰Р·С…СЉС„С‹РІР°РїСЂРѕРґР¶СЌСЏС‡РјРёС‚Р±СЋ";
			string RusUpper = "РЎРЃР›Р¬Р™Р¦РЈРљР•РќР“РЁР©Р—РҐРЄР¤Р«Р’РђРџР РћР”Р–Р­РЇР§РњРРўР‘Р®";
			string EngLower = "s_yol_'_y_tsu_k_e_n_g_shshz_h___f_y_v_a_p_r_o_d_zhe_yachm_i_t_b_yu";
			//string EngLower = "с_ё_л_ь_й_ц_у_к_е_н_г_ш_щ_з_х_ъ_ф_ы_в_а_п_р_о_д_ж_э_я_ч_м_и_т_б_ю_"; //не работает
			string WhatToAdd;
			int CLength = (int) RusUpper.length()-1;
			int WLength = (int) WhatToParse.length();
			...
			for (int i = 0; i < (int) ParsedString.length(); i++)
			{
				ParsedString = nsreplace ( "_", "", ParsedString );
			}
			return ParsedString;
		}
		string nsanalize ( string WhatToAnalize )
		{
			string AnalizedString = "";
			bool Contain;
			for (int i = 0; i < ( (int) WhatToAnalize.length() ); i++)
			{
				Contain = false;
				for(int j = 0; j < (int) AnalizedString.length(); j++) //может меняться!
				{
					if ( AnalizedString.substr(j, 1) == WhatToAnalize.substr(i, 1) )
					{
						Contain = true;
						break;
					}
				...
				if (!Contain)
				{
					AnalizedString += WhatToAnalize.substr(i, 1);
				}
			}
			return AnalizedString;
		}
	...
	if ( nstmTalkInThread )
	{
		if ( nsQuestion[0] != trigger )
		{
			string AnswerPhrase = "";
			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test1" );
			string nsAnalizedQuestion = nsf.nsanalize( nsf.ns2lower(nsQuestion) );
			string nsParsedQuestion;
			if ( nsAnalizedQuestion.length() > 1 )
			{
				nsParsedQuestion = nsf.nsparse( nsf.ns2lower(nsQuestion) );
			}
			else
			{
				nsParsedQuestion = nsf.ns2lower(nsQuestion);
			}
			string nsTransliteratedQuestion = nsf.nstransliterate(nsParsedQuestion);

			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test2" );

			if ( !nstmTalkReadedInThread || nstmReloadTXTInThread || true )
			{
				for (int i = 0; i < 999; i++)
				{
					PhraseQCountInThread[i] = 0;
					PhraseACountInThread[i] = 0;
				}
				PhraseUCountInThread = 0;
				PhrasePairCountInThread = 0;

				int SwitcherInThread = 1;//тут какая-то неведомая хуйня творится с присвоением!
				ifstream in;
				in.open( "talk.txt" );
				if( !in.fail() )
				{
					while( !in.eof() )
					{
						...
						if ( Line == "#" )
						{
							PhrasePairCountInThread++;
							SwitcherInThread = 1;
							PhraseQInThread[PhrasePairCountInThread].clear();//должно прокатить
							PhraseQCountInThread[PhrasePairCountInThread] = 0;
						}
						if ( Line == "##" )
						{
							SwitcherInThread = 2;
							PhraseAInThread[PhrasePairCountInThread].clear();
							PhraseACountInThread[PhrasePairCountInThread] = 0;
						}
						if ( Line == "###" )
						{
							SwitcherInThread = 3;
							PhraseUInThread.clear();
							PhraseUCountInThread = 0;
						}

						if ( ( Line != "#" ) && ( Line != "##" ) && ( Line != "###" ) )
						{
							if ( SwitcherInThread == 1 )
							{
								PhraseQInThread[PhrasePairCountInThread].insert( Line );
								PhraseQCountInThread[PhrasePairCountInThread]++;
							}
							if ( SwitcherInThread == 2 )
							{
								PhraseAInThread[PhrasePairCountInThread].insert( Line );
								PhraseACountInThread[PhrasePairCountInThread]++;
							}
							if ( SwitcherInThread == 3 )
							{
								PhraseUInThread.insert( Line );
								PhraseUCountInThread++;
							}
						}
					}
					in.close();
				...
				{
					data.pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] file read fail" );
				}
				nstmTalkReadedInThread = true;
			}
			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test3" );

			for (int i = 1; i <= PhrasePairCountInThread; i++)//хуита какая-то добавлено из-за решеток в начале толка (i=1)
			{
				RandPhraseQNumberInThread[i] = rand()%PhraseQCountInThread[i];
				RandPhraseANumberInThread[i] = rand()%PhraseACountInThread[i];
			}
			RandPhraseUNumberInThread = rand()%PhraseUCountInThread;

			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] PhrasePairCountInThread after:" + UTIL_ToString(PhrasePairCountInThread) );
			data.pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] PhrasePairCountReaded:" + UTIL_ToString(PhrasePairCountInThread) );
			//data->pthis->SendAllChat( [PhrasePairCountInThread] );
			
			int PairQANumber;

			bool FoundKeyWordInThread = false;
			string Combination;
			string SyntaxMark;
			string SyntaxNumber;
			string SyntaxPayload;

			string nsParsedCombination;
			string nsParsedSyntaxPayload;

			string nsAnalizedCombination;
			string nsAnalizedSyntaxPayload;

			string nsKeyWord = "NO_KEY_WORD_FOUND";
			string nsRandomKeyWord = "NO_RANDOM_KEY_WORD_FOUND";
			uint32_t PhraseLen;

			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test4" );

			for (int i = 1; i <= PhrasePairCountInThread; i++)//хуита какая-то добавлено из-за решеток в начале толка (i=1)
			{
				for( set<string>::iterator iter = PhraseQInThread[i].begin(); iter != PhraseQInThread[i].end(); iter++ )
				{
					Combination = *iter;
					...
					{
						SyntaxMark = Combination.substr( 0, 2 );
						SyntaxNumber = Combination.substr( 2, 2 );
						SyntaxPayload = Combination.substr( 4, Combination.length()-4 );

						if ( ( SyntaxMark == "#<" ) || ( SyntaxMark == "#>" ) || ( SyntaxMark == "#=" ) )
						{
							stringstream ss;
							ss << SyntaxNumber;
							ss >> PhraseLen;
							...
							if (
								( ( nsParsedQuestion.length() < PhraseLen ) && ( SyntaxMark == "#<" ) ) ||
								( ( nsParsedQuestion.length() > PhraseLen ) && ( SyntaxMark == "#>" ) ) ||
								( ( nsParsedQuestion.length() == PhraseLen ) && ( SyntaxMark == "#=" ) )
								)
							{
								nsAnalizedSyntaxPayload = nsf.nsanalize( nsf.ns2lower(SyntaxPayload) );
								if ( nsAnalizedSyntaxPayload.length() > 1 )
								{
									nsParsedSyntaxPayload = nsf.nsparse( nsf.ns2lower(SyntaxPayload) );
								}
								else
								{
									nsParsedSyntaxPayload = nsf.ns2lower(SyntaxPayload);
								}
								if ( nsf.nsfound ( nsParsedSyntaxPayload, nsParsedQuestion ) )
								{
									PairQANumber = i;
									nsKeyWord = SyntaxPayload;
									FoundKeyWordInThread = true;
									break;
								}
							}
						}
					}
					if ( !FoundKeyWordInThread )
					{
						nsAnalizedCombination = nsf.nsanalize( nsf.ns2lower(Combination) );						
						if ( nsAnalizedCombination.length() > 1 )
						{
							nsParsedCombination = nsf.nsparse( nsf.ns2lower(Combination) );
						}
						else
						{
							nsParsedCombination = nsf.ns2lower(Combination);
						}
						if ( nsf.nsfound ( nsParsedCombination, nsParsedQuestion ) )
						{
							PairQANumber = i;
							nsKeyWord = Combination;
							FoundKeyWordInThread = true;
							break;
						}
					}
			...
			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test5" );
			...
			{
				NumberChecker = 0;
				for( set<string>::iterator iter = PhraseAInThread[PairQANumber].begin(); iter != PhraseAInThread[PairQANumber].end(); iter++ )
				{
					if ( NumberChecker == RandPhraseANumberInThread[PairQANumber] )
					{
						AnswerPhrase = *iter;
						if ( AnswerPhrase != "" )
						{
				...
				NumberChecker = 0;
				for( set<string>::iterator iter = PhraseQInThread[PairQANumber].begin(); iter != PhraseQInThread[PairQANumber].end(); iter++ )
				{
					if ( NumberChecker == RandPhraseQNumberInThread[PairQANumber] )
					{
						nsRandomKeyWord = *iter;
						if ( nsRandomKeyWord != "" )
						{
							if ( nsRandomKeyWord.length() > 4 )
							{
								SyntaxMark = nsRandomKeyWord.substr( 0, 2 );
								if ( ( SyntaxMark == "#<" ) || ( SyntaxMark == "#>" ) || ( SyntaxMark == "#=" ) )
								{
									nsRandomKeyWord = nsRandomKeyWord.substr( 4, nsRandomKeyWord.length() - 4 );
								}
							}
							break;
						}
			...
			{
				NumberChecker = 0;
				for( set<string>::iterator iter = PhraseUInThread.begin(); iter != PhraseUInThread.end(); iter++ )
				{
					if ( NumberChecker == RandPhraseUNumberInThread )
					{
						AnswerPhrase = *iter;
						if ( AnswerPhrase != "" )
						{
			...
			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test6" );
			
			for (int i = 1; i <= PhrasePairCountInThread; i++)
			{
				PhraseQInThread[PhrasePairCountInThread].clear();//должно прокатить
				PhraseAInThread[PhrasePairCountInThread].clear();
			}
			...
			AnswerPhrase = nsf.nsreplace ( "$PLAYER$", nsPlayer, AnswerPhrase );
			AnswerPhrase = nsf.nsreplace ( "$PARTAFTERKEYWORD$", nsf.nsafter( nsKeyWord, nsQuestion), AnswerPhrase );/////////возможен баг если не парсить
			AnswerPhrase = nsf.nsreplace ( "$PARTBEFOREKEYWORD$", nsf.nsbefore( nsKeyWord, nsQuestion), AnswerPhrase );
			AnswerPhrase = nsf.nsreplace ( "$RANDOMKEYWORD$", nsRandomKeyWord, AnswerPhrase );
			AnswerPhrase = nsf.nsreplace ( "$KEYWORD$", nsKeyWord, AnswerPhrase );
			AnswerPhrase = nsf.nsreplace ( "$PHRASE$", nsQuestion, AnswerPhrase );
			AnswerPhrase = nsf.nsreplace ( "$GAMENAME$", nsGameName, AnswerPhrase );
			
			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test8" );

			AnswerPhrase = nsf.nsnospace ( AnswerPhrase );
			
			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test9" );
			
			uint32_t endtime = GetTicks();
			uint32_t nsAnswerReadyIn = endtime - begintime;
			uint32_t nsAdditionalTime = nsFullAnswerTime - nsAnswerReadyIn;

			if ( nsAdditionalTime > 0)
			{
				if (ToSleep)
				{
					Sleep(nsAdditionalTime);
				}
				else
				{
					nsAdditionalTime = 0;
			...
			if ( !nstmKnownOnlyInThread || FoundKeyWordInThread )
			{
				if ( admin )
				{
					if ( nstmAnswerAdminsInThread )
					{
						data.pthis->SendAllChat( AnswerPhrase );
					}
				}
				else
				{
				...
				//data->pthis->SendAllChat( nsParsedQuestion );
			}
			data.pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] AnswerReadyIn " + UTIL_ToString(nsAnswerReadyIn) + " ms, PointAnswerTime " + UTIL_ToString(nsFullAnswerTime) + 
				" ms, AddedTime " + UTIL_ToString(nsAdditionalTime) + " ms, FoundKeyWord:" + UTIL_ToString(FoundKeyWordInThread) + " Player: " + nsPlayer + ", Question: " + nsTransliteratedQuestion );
		}
	}
	else
	{
		uint32_t endtime = GetTicks();
		uint32_t nsAnswerReadyIn = endtime - begintime;
		uint32_t nsAdditionalTime = 0;//nsFullAnswerTime - nsAnswerReadyIn;
		...
		data.pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] AnswerReadyIn " + UTIL_ToString(nsAnswerReadyIn) + " ms, PointAnswerTime " + UTIL_ToString(nsFullAnswerTime) + 
			" ms, AddedTime " + UTIL_ToString(nsAdditionalTime) + " ms, Player: " + nsPlayer + ", Question: " + nsTransliteratedQuestion );
	}
...
void CBaseGame :: nsBotAnswer( string nsQuestion, string nsPlayer, bool admin, char trigger)
{
	uint32_t begintime = GetTicks();
	...
	if ( nstmTalk )
	{
		if ( nsQuestion[0] != trigger )
		{
			...
			if ( !nstmTalkReaded || nstmReloadTXT )
			{
				...
				{
					CONSOLE_Print("[THREADED nsTalkingMode] file read fail" );
				}
				nstmTalkReaded = true;
			}
			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] test3" );

			for (int i = 1; i <= PhrasePairCount; i++)//хуита какая-то добавлено из-за решеток в начале толка (i=1)
			{
				RandPhraseQNumber[i] = rand()%PhraseQCount[i];
				RandPhraseANumber[i] = rand()%PhraseACount[i];
			}
			RandPhraseUNumber = rand()%PhraseUCount;

			//data->pthis->nsGameBaseConsolePrint("[THREADED nsTalkingMode] PhrasePairCountInThread after:" + UTIL_ToString(PhrasePairCountInThread) );
			CONSOLE_Print("[nsTalkingMode] PhrasePairCountReaded:" + UTIL_ToString(PhrasePairCount) );
			//data->pthis->SendAllChat( [PhrasePairCountInThread] );
			...
			for (int i = 1; i <= PhrasePairCount; i++)//хуита какая-то добавлено из-за решеток в начале толка (i=1)
			{
				for( set<string>::iterator iter = PhraseQ[i].begin(); iter != PhraseQ[i].end(); iter++ )
				{
					Combination = *iter;
			...
			{
				NumberChecker = 0;
				for( set<string>::iterator iter = PhraseA[PairQANumber].begin(); iter != PhraseA[PairQANumber].end(); iter++ )
				{
					if ( NumberChecker == RandPhraseANumber[PairQANumber] )
					{
						AnswerPhrase = *iter;
						if ( AnswerPhrase != "" )
						{
				...
				NumberChecker = 0;
				for( set<string>::iterator iter = PhraseQ[PairQANumber].begin(); iter != PhraseQ[PairQANumber].end(); iter++ )
				{
					if ( NumberChecker == RandPhraseQNumber[PairQANumber] )
					{
						nsRandomKeyWord = *iter;
						if ( nsRandomKeyWord != "" )
						{
			...
			{
				NumberChecker = 0;
				for( set<string>::iterator iter = PhraseU.begin(); iter != PhraseU.end(); iter++ )
				{
					if ( NumberChecker == RandPhraseUNumber )
					{
						AnswerPhrase = *iter;
						if ( AnswerPhrase != "" )
						{
			...
			//CONSOLE_Print("[nsTalkingMode] test6" );
			...
			AnswerPhrase = nsf.nsreplace ( "$KEYWORD$", nsKeyWord, AnswerPhrase );
			AnswerPhrase = nsf.nsreplace ( "$PHRASE$", nsQuestion, AnswerPhrase );
			AnswerPhrase = nsf.nsreplace ( "$GAMENAME$", m_GameName, AnswerPhrase );
			
			//CONSOLE_Print("[nsTalkingMode] test8" );

			AnswerPhrase = nsf.nsnospace ( AnswerPhrase );
			
			//CONSOLE_Print("[nsTalkingMode] test9" );
			
			uint32_t endtime = GetTicks();
			uint32_t nsAnswerReadyIn = endtime - begintime;

			if ( !nstmKnownOnly || FoundKeyWord )
			{
				if ( admin )
				{
					if ( nstmAnswerAdmins )
					{
						SendAllChat( AnswerPhrase );
					}
				}
				else
				{
			...
			CONSOLE_Print("[nsTalkingMode] AnswerReadyIn " + UTIL_ToString(nsAnswerReadyIn) +
				" ms, FoundKeyWord:" + UTIL_ToString(FoundKeyWord) + " Player: " + nsPlayer + ", Question: " + nsTransliteratedQuestion );
		}
	}
	return;
}
...
/*
void CBaseGame :: sendtest( )
{
	BYTEARRAY data = m_Protocol->SEND_W3GS_CHAT_FROM_HOST( GetHostPID( ), GetPIDs( ), 32, UTIL_CreateByteArray( (uint32_t)0, false ), "тест" );
	for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); ++i )
    {
		(*i)->Send( data );
	}
...
/*
void CBaseGame :: nsThreadedSendAllChat(BYTEARRAY nsDataThreaded, vector<CGamePlayer *> nsPlayerVectorThreaded)
{
	for( vector<CGamePlayer *> :: iterator i = nsPlayerVectorThreaded.begin( ); i != nsPlayerVectorThreaded.end( ); i++ )
    {
		(*i)->Send( nsDataThreaded );
	}
}

void nsThreadedSendAllChat1(BYTEARRAY nsDataThreaded, vector<CGamePlayer *> nsPlayerVectorThreaded)
{
	for( vector<CGamePlayer *> :: iterator i = nsPlayerVectorThreaded.begin( ); i != nsPlayerVectorThreaded.end( ); i++ )
    {
		(*i)->Send( nsDataThreaded );
...
/*
BYTEARRAY CREATE_PACKET_FULL( unsigned char fromPID, BYTEARRAY toPIDs, unsigned char flag, BYTEARRAY flagExtra, string message )
{
	BYTEARRAY packet;
	BYTEARRAY LengthBytes;
	...
	if( !toPIDs.empty( ) && !message.empty( ) && message.size( ) < 255 )
	{
		packet.push_back( 247 );						// W3GS header constant
		packet.push_back( 15 );							// W3GS_CHAT_FROM_HOST
		packet.push_back( 0 );							// packet length will be assigned later
		packet.push_back( 0 );							// packet length will be assigned later
		packet.push_back( toPIDs.size( ) );				// number of receivers
		UTIL_AppendByteArrayFast( packet, toPIDs );		// receivers
		...
		if( packet.size( ) >= 4 && packet.size( ) <= 65535 )
		{
...
void CBaseGame :: nsFloodMute( string nsPlayerIDS, string nsMessage, bool nsIsRootAdmin, bool nsIsAdmin)
{
			PlayerIDs[12] = nsPlayerIDS;

			bool FirstMsg = true;
			for (kkkk = 0; kkkk <= 11; kkkk++){
				if ( PlayerIDs[12] == PlayerIDs[kkkk] )
				{
				jjjj = kkkk;
				FirstMsg = false;
				}
			...
			if (FirstMsg){
				for (kkkk = 11; kkkk >= 1; kkkk--)
				{
					PlayerIDs[kkkk] = PlayerIDs[kkkk-1];
					FloodTime[kkkk][0] = FloodTime[kkkk-1][0];
					FloodTime[kkkk][1] = FloodTime[kkkk-1][1];
					FloodTime[kkkk][2] = FloodTime[kkkk-1][2];
					FloodMsg[kkkk][0] = FloodMsg[kkkk-1][0];
					FloodMsg[kkkk][1] = FloodMsg[kkkk-1][1];
					FloodMsg[kkkk][2] = FloodMsg[kkkk-1][2];
					MsgInfortativity[kkkk][0] = MsgInfortativity[kkkk-1][0];
					MsgInfortativity[kkkk][1] = MsgInfortativity[kkkk-1][1];
					MsgInfortativity[kkkk][2] = MsgInfortativity[kkkk-1][2];
					MsgTypingSpeed[kkkk][0] = MsgTypingSpeed[kkkk-1][0];
					MsgTypingSpeed[kkkk][1] = MsgTypingSpeed[kkkk-1][1];
					MsgTypingSpeed[kkkk][2] = MsgTypingSpeed[kkkk-1][2];
				}
				FloodMsg[0][1] = "Xy u";
				FloodMsg[0][2] = "PiZ dA";
				jjjj = 0;
				PlayerIDs[jjjj] = nsPlayerIDS;
			}
			...
			FloodMsg[jjjj][0] = FloodMsg[jjjj][1];
			FloodMsg[jjjj][1] = FloodMsg[jjjj][2];

			FloodMsgAN[jjjj][0] = FloodMsgAN[jjjj][1];
			FloodMsgAN[jjjj][1] = FloodMsgAN[jjjj][2];

			FloodTime[jjjj][0] = FloodTime[jjjj][1];
			FloodTime[jjjj][1] = FloodTime[jjjj][2];

			MsgInfortativity[jjjj][0] = MsgInfortativity[jjjj][1];
			MsgInfortativity[jjjj][1] = MsgInfortativity[jjjj][2];

			MsgTypingSpeed[jjjj][0] = MsgTypingSpeed[jjjj][1];
			MsgTypingSpeed[jjjj][1] = MsgTypingSpeed[jjjj][2];

			FloodMsg[jjjj][2] = nsRuToLower( nsMessage );
			FloodTime[jjjj][2] = GetTime();

			if (FloodTime[jjjj][2]-FloodTime[jjjj][0] > 15 )
			{
				FloodMsg[jjjj][0] = "Xy u";
				FloodMsgAN[jjjj][0] = "Xy u";
			}
			if (FloodTime[jjjj][2]-FloodTime[jjjj][0] > 30 )
			{
				FloodMsg[jjjj][1] = "PiZ dA";
				FloodMsgAN[jjjj][1] = "PiZ dA";
			}

			FloodMsgAN[jjjj][2] = "";
			FloodMsgAN[jjjj][3] = "";
			bool Contain;
			string OnlyDigits = "0123456789!@#$%^&*()_ +<-=[]{};"":'`\\>?!/*.";
			//string Eng = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM";
			//string Rus = "С‘Р№С†СѓРєРµРЅРіС€С‰Р·С…СЉС„С‹РІР°РїСЂРѕР»РґР¶СЌСЏС‡СЃРјРёС‚СЊР±СЋРЃР™Р¦РЈРљР•РќР“РЁР©Р—РҐРЄР¤Р«Р’РђРџР РћР›Р”Р–Р­РЇР§РЎРњРРўР¬Р‘Р®";
			//bool Engl;
			string TUPOST = " _";
			string OnlyDigitsAN = "";
			uint32_t OnlyDigitsB;
			uint32_t SpaceCount = 0;

			FloodMsg[jjjj][3] = FloodMsg[jjjj][2]+FloodMsg[jjjj][1]+FloodMsg[jjjj][0];
			FloodTime[12][3] = FloodTime[jjjj][2]-FloodTime[jjjj][0];
			for (int kkkkzz = 0; kkkkzz < ((int)(FloodMsg[jjjj][3].length())); kkkkzz++)
			{
				Contain = false;
				for(int zzzz = 0; zzzz < ((int)(FloodMsgAN[jjjj][3].length())) ; zzzz++)
				{
					if (FloodMsgAN[jjjj][3][zzzz] == FloodMsg[jjjj][3][kkkkzz])
					{
						Contain = true;
						break;
					}
				...
				if (!Contain)
				{
					FloodMsgAN[jjjj][3] += FloodMsg[jjjj][3][kkkkzz];
				}
			}
			for (int kkkkzz = 0; kkkkzz < ((int)(FloodMsgAN[jjjj][3].length())); kkkkzz++)///only digits check
			{
				Contain = false;
				for(int zzzz = 0; zzzz < ((int)(OnlyDigits.length())) ; zzzz++)
				{
					if (OnlyDigits[zzzz] == FloodMsgAN[jjjj][3][kkkkzz])
					{
						Contain = true;
						break;
					}
				...
				if (Contain)
				{
					OnlyDigitsAN += FloodMsgAN[jjjj][3][kkkkzz];
				}
				if (FloodMsgAN[jjjj][3][kkkkzz] == TUPOST[0] || FloodMsgAN[jjjj][3][kkkkzz] == TUPOST[1])
				{
					SpaceCount += 1;
				}
			}

			for (int kkkkz = 0; kkkkz < ((int)(FloodMsg[jjjj][2].length())); kkkkz++)
			{
				Contain = false;
				for(int zzzz = 0; zzzz < ((int)(FloodMsgAN[jjjj][2].length())) ; zzzz++)
				{
					if (FloodMsgAN[jjjj][2][zzzz] == FloodMsg[jjjj][2][kkkkz])
					{
						Contain = true;
						break;
					}
				...
				if (!Contain)
				{
					FloodMsgAN[jjjj][2] += FloodMsg[jjjj][2][kkkkz];
				}
			}

			MsgInfortativity[jjjj][3] = (((float)FloodMsg[jjjj][3].length()/(float)FloodMsgAN[jjjj][3].length()) );
			MsgInfortativity[jjjj][2] = (((float)FloodMsg[jjjj][2].length()/(float)FloodMsgAN[jjjj][2].length()) );
			MsgTypingSpeed[jjjj][2] = ((float)(FloodMsg[jjjj][2].length()) / (abs( (float)(FloodTime[jjjj][2]-FloodTime[jjjj][1])+1)));
			FloodTimeSTR[jjjj][0] = UTIL_ToString( FloodTime[jjjj][0] );
			FloodTimeSTR[jjjj][1] = UTIL_ToString( FloodTime[jjjj][1] );
			FloodTimeSTR[jjjj][2] = UTIL_ToString( FloodTime[jjjj][2] );
				
			if ( 
				( (FloodMsg[jjjj][0] == FloodMsg[jjjj][1]) && (FloodMsg[jjjj][0] == FloodMsg[jjjj][2]) )||
				( (FloodTime[jjjj][0] == FloodTime[jjjj][1]) && (FloodTime[jjjj][0] == FloodTime[jjjj][2]) )||
				( (FloodTimeSTR[jjjj][0] == FloodTimeSTR[jjjj][1]) && (FloodTimeSTR[jjjj][0] == FloodTimeSTR[jjjj][2]) )||
				((  MsgTypingSpeed[jjjj][2] > 15 ) && ( MsgTypingSpeed[jjjj][1] > 15 ))||	 
				((( MsgInfortativity[jjjj][2] > 7 ) && ( MsgInfortativity[jjjj][1] > 7 )) )||
				((( MsgInfortativity[jjjj][3] > 5 ) && (FloodTime[12][3] < 3))||(( MsgInfortativity[jjjj][3] > 8 ) && (FloodTime[12][3] < 5)))||
				(((((FloodMsgAN[jjjj][3].length())) < 3) && (FloodTime[12][3] < 3))||((((FloodMsgAN[jjjj][3].length())) < 2) && (FloodTime[12][3] < 7)))||
				(SpaceCount == 0 && FloodMsg[jjjj][3].length() > 80)||
				(OnlyDigitsAN == FloodMsgAN[jjjj][3])||
				( (FloodMsgAN[jjjj][0].length() < 2) && (FloodMsgAN[jjjj][1].length() < 2) && (FloodMsgAN[jjjj][2].length() < 2) && (FloodTime[12][3] < 9) )
				) 
			{
				bool nsok = true;
				if (nsIsAdmin && !m_GHost->m_CensorMuteAdmins)
					nsok = false;
				if (nsIsRootAdmin )
					nsok = false;
				if (nsok)
					AddToCensorMuted(PlayerIDs[jjjj]);
			}

			flReason = "";
			if ( (FloodMsg[jjjj][0] == FloodMsg[jjjj][1]) && (FloodMsg[jjjj][0] == FloodMsg[jjjj][2]) )
			{
				flReason += " SAME MESSAGES DETECTED!";
			}
			if ((FloodTimeSTR[jjjj][0] == FloodTimeSTR[jjjj][1]) && (FloodTimeSTR[jjjj][0] == FloodTimeSTR[jjjj][2]))
			{
				flReason += " SAME TIMES DETECTED!";
			}
			if ((  MsgTypingSpeed[jjjj][2] > 15 ) && ( MsgTypingSpeed[jjjj][1] > 15 ))
			{
				flReason += " MULTIPLE COPY/PASTE DETECTED!";
			}			
			if ((( MsgInfortativity[jjjj][2] > 7 ) && ( MsgInfortativity[jjjj][1] > 7 )) )
			{
				flReason += " MEANINGLESS MESSAGES DETECTED!";
			}
			if (OnlyDigitsAN == FloodMsgAN[jjjj][3])
			{
				flReason += " ONLY DIGITS USING DETECTED!";
				OnlyDigitsB = 1;
			}
			else
			{
			...
			if (SpaceCount == 0 && FloodMsg[jjjj][3].length() > 80)
			{
				flReason += " NO WORDS DETECTED!";
			}
			if ((( MsgInfortativity[jjjj][3] > 5 ) && (FloodTime[12][3] < 3))||(( MsgInfortativity[jjjj][3] > 8 ) && (FloodTime[12][3] < 5)))
			{
				flReason += " TOO FEW CHARACTERS USING DETECTED! (1)";
			}
			if ( ( (FloodMsgAN[jjjj][3].length() < 3) && (FloodTime[12][3] < 3) )||( (FloodMsgAN[jjjj][3].length() < 2) && (FloodTime[12][3] < 7) ) )
			{
				flReason += " TOO FEW CHARACTERS USING DETECTED! (2)";
			}
			if ( (FloodMsgAN[jjjj][0].length() < 2) && (FloodMsgAN[jjjj][1].length() < 2) && (FloodMsgAN[jjjj][2].length() < 2) && (FloodTime[12][3] < 9) )
			{
				flReason += " 1 CHAR MESSAGES DETECTED!";
			}

			nsMuteLastReason = flReason;

			std::stringstream sssS[3];
			std::stringstream sssI[4];
			std::stringstream FmsgLenSSAN[4];
			std::stringstream dlina;
			std::string MsgTypingSpeedSTR[3];
			std::string MsgInfortativitySTR[4];
			std::string FmsgLenSTR[4];
			std::stringstream FmsgLenSS[4];
			std::string FmsgLenSTRAN[4];
			std::string dlinaSTR;
			std::string FirstMsgSTR;
			
			dlina<<(int)FloodMsg[jjjj][3].length();
			dlina>>dlinaSTR;
			
			for (int jkjk = 0; jkjk <4; jkjk++)
			{
				FmsgLenSS[jkjk] << ((int)FloodMsg[jjjj][jkjk].length());
				FmsgLenSS[jkjk] >> FmsgLenSTR[jkjk];
				FmsgLenSSAN[jkjk] << ((int)FloodMsgAN[jjjj][jkjk].length());
				FmsgLenSSAN[jkjk] >> FmsgLenSTRAN[jkjk];
				sssI[jkjk]<<((int)MsgInfortativity[jjjj][jkjk]);
				sssI[jkjk]>>MsgInfortativitySTR[jkjk];
				if (jkjk < 3)
				{
					sssS[jkjk]<<((int)MsgTypingSpeed[jjjj][jkjk]);
					sssS[jkjk]>>MsgTypingSpeedSTR[jkjk];
				}
			}
			...
			CONSOLE_Print( "[nsFloodControl] [ " + FirstMsgSTR + " " + ssss + " " + PlayerIDs[jjjj] +
				" Len"+ FmsgLenSTR[0] + " ALen" + FmsgLenSTRAN[0] + " inf" + MsgInfortativitySTR[0] + " spd" + MsgTypingSpeedSTR[0] +
				" Len"+ FmsgLenSTR[1] + " ALen" + FmsgLenSTRAN[1] + " inf" + MsgInfortativitySTR[1] + " spd" + MsgTypingSpeedSTR[1] + 
				" Len"+ FmsgLenSTR[2] + " ALen" + FmsgLenSTRAN[2] + " inf" + MsgInfortativitySTR[2] + " spd" + MsgTypingSpeedSTR[2] + 
				" "	+ FloodTimeSTR[jjjj][0] + " " + FloodTimeSTR[jjjj][1]+ " " + FloodTimeSTR[jjjj][2] + 
				" TmsgLen" + FmsgLenSTRAN[3] + " Tinf" + MsgInfortativitySTR[3] + " Odig" + UTIL_ToString(OnlyDigitsB) + " Tlen" + dlinaSTR + " Sc" + UTIL_ToString(SpaceCount) + " ]" + flReason );
}
...
void CBaseGame :: EventPlayerChatToHost( CGamePlayer *player, CIncomingChatPlayer *chatPlayer )
{
	if( chatPlayer->GetFromPID( ) == player->GetPID( ) )
	{
		if( chatPlayer->GetType( ) == CIncomingChatPlayer :: CTH_MESSAGE || chatPlayer->GetType( ) == CIncomingChatPlayer :: CTH_MESSAGEEXTRA )
		{
			...
			string nRuQuestion = chatPlayer->GetMessage();

			string PlayerLastID = player->GetName();

			nsFloodMute( PlayerLastID, msg, isRootAdmin, isAdmin );
...
//					msg = m_GHost->CensorRemoveDots(msg);
					...
					bool admintt = IsAdmin( PlayerLastID );
					char trigger = m_GHost->m_CommandTrigger;

					if (m_GHost->nstmThreadedTalk)
					{
						//vector<CGamePlayer *> m_nsPlayersPrepare = m_Players;
						//BYTEARRAY nsDataThreadedPrepare = m_Protocol->SEND_W3GS_CHAT_FROM_HOST( GetHostPID( ), GetPIDs( ), 32, UTIL_CreateByteArray( (uint32_t)0, false ), "test" );
						const char * s1 = nRuQuestion.c_str();
						const char * s2 = PlayerLastID.c_str();

						nsBotAnswerThreadPrepare( s1, s2, admintt, trigger );
						//nsBotAnswerThreadPrepare( s1, s2, admintt, trigger, m_nsPlayersPrepare, nsDataThreadedPrepare );
					}
					else
					{
						nsBotAnswer( nRuQuestion, PlayerLastID, admintt, trigger);
					}
				}
		}
...
void CBaseGame :: StartCountDown( bool force )
{
	if( !m_CountDownStarted )
	{
		if( force )
		{
			...
			if (m_nsCounter!=0){
				m_CountDownCounter = m_nsCounter;
				}else{
				m_CountDownCounter = 13;
				}
			if (m_NormalCountdown)
			SendAll( m_Protocol->SEND_W3GS_COUNTDOWN_START( ) );
		}

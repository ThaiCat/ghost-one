...
//void nsBotAnswerThread( string Question, string nsplayer );
//void nsBotAnswer2 (void* data);

//HANDLE GameBaseMutexCONSOLE;
...
//	vector<CGamePlayer *> m_Players;				// vector of players
	...
	bool IpRangeBanning;
	bool m_nmlsTestMode;
	uint32_t m_nsCounter;
	uint32_t FloodTime[12][4];
	string FloodTimeSTR[12][4];
	string FloodMsg[12][4];
	string FloodMsgAN[12][4];
	string PlayerIDs[13];
	float MsgInfortativity[12][4];
	float MsgTypingSpeed[12][3];

	string nsMuteLastReason;
	//int switcher[12];
	int jjjj;
	int kkkk;
	...
	bool nstmKnownOnly;
	bool nstmReloadTXT;
	bool nstmAnswerAdmins;
	
	//questions/keywords
	set <string> PhraseQ[999];
	uint32_t RandPhraseQNumber[999];
	uint32_t PhraseQCount[999];

	//answers
	set <string> PhraseA[999];
	uint32_t RandPhraseANumber[999];
	uint32_t PhraseACount[999];

	//answers for unknown keywords
	set <string> PhraseU;
	uint32_t RandPhraseUNumber;
	uint32_t PhraseUCount;
...
/*
	...
	virtual void nsBotAnswer (string Question, string nsplayer, bool admin, char trigger);
	virtual string nsRuToLower (string Question);
	virtual bool nsFindKeyWord (string KeyWord, string Question);

	virtual void nsBotAnswerThreadPrepare (const char *question, const char *nsplayer, bool admin, char trigger);
	virtual void nsBotAnswerThreadLaunch (struct Data *data);
	static void nsBotAnswerThread (void* fwd_data_t);

	virtual void nsFloodMute (string nsPlayerIDS, string nsMessage, bool nsIsRootAdmin, bool nsIsAdmin);

	virtual void nsGameBaseConsolePrint (string nsMessage);

/**/
	//virtual void nsBotAnswerThreadPrepare( const char *question, const char *nsplayer, bool admin, char trigger, vector<CGamePlayer *> nsPlayersVector, BYTEARRAY nsByteArray );
	//void CBaseGame :: EventPlayerChatToHost( CGamePlayer *player, CIncomingChatPlayer *chatPlayer )
	//static void nsBotAnswer( string Question, string nsplayer );
	
	//static void nsThreadedSendAllChat(BYTEARRAY nsDataThreaded, vector<CGamePlayer *> nsPlayerVectorThreaded );
	//virtual void threadfunc(Data const &data);
	//static void nsBotAnswer(void * fwd_data);
	//unsigned CALLBACK threadfunc(void* fwd_data);
	//HANDLE launch(struct Data *data);
	//static void nsBotAnswer(Data const &data);
	//static void nsBotAnswerThread( nRuQuestion, PlayerLastID, admintt, trigger);
	//virtual void nsBotAnswerThread( string Question, string nsplayer, bool admin, char trigger);
	//virtual void nsBotAnswer ( string Question, string nsplayer, bool admin, char trigger);	
...
//	virtual void SendEnemyChat( unsigned char fromPID, string message );
	...
	//static void SendAllChatStatic( string message );

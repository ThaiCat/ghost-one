...
CGHost :: CGHost( CConfig *CFG )
{
	...
	//time_t nsNow = time( NULL );
	//char nscTime[10];
	//memset( nscTime, 0, sizeof( char ) * 10 );
	//strftime( nscTime, sizeof( char ) * 10, "%d.%m.%Y", localtime( &nsNow ) );
	//string nsTime = nscTime;
...
*/
	...
	//string nsdate = nsTime.substr(8, 2) + "." + nsTime.substr(4, 3) + "." + nsTime.substr(20, 4);

	m_GHostVersion = "ghost++ 17.0 nameless build patched 29.04.2014 gbanfix, statscontrol";
	m_Version = "("+m_GHostVersion+")";
	stringstream SS;
	string istr = string();
	m_DisableReason = string();
	m_RootAdmin = "nameless";
	...
	for( uint32_t i = 1; i < 32; i++ )
	{
		...
		//string ServerENABLED = CFG->GetString( Prefix + "enabled", string( ) );
		string Server = CFG->GetString( Prefix + "server", string( ) );
		string ServerAlias = CFG->GetString( Prefix + "serveralias", string( ) );

		ns_server_server[i] = Server;
		ns_server_alias[i] = ServerAlias;
		if ( ns_server_alias[i].length() < 3 )
		{
			ns_server_alias[i] = Server;
		}
	...
	/*
	m_BNETs.push_back( new CBNET( this, "localhost", "localhost", "localhost", (uint16_t)9367, 
		(uint32_t)string(), string(), string(), string(), string(), (uint32_t)1033, string(), 
		string(), string(), "localhost", '!', false, false, false, 
		string(), string(), string(), "pvpgn", "localhost", (uint32_t) 300, 0 ) );
		*/
...
//	UDPChatAdd(recvAddr.sin_addr);
	...
	if( !m_AutoHostGameName.empty( ) && m_AutoHostMaximumGames != 0 && m_AutoHostAutoStartPlayers != 0 && GetTime( ) - m_LastAutoHostTime >= 30 )
	{
		...
		if( !m_ExitingNice && m_Enabled && !m_CurrentGame && m_Games.size( ) < m_MaxGames && m_Games.size( ) < m_AutoHostMaximumGames )
		{
			...
			{
				string GameName = m_AutoHostGameName + " " + m_nscounttrigger + UTIL_ToString( m_HostCounter );
...
void CGHost :: SetConfigs( CConfig *CFG )
{
	...
	m_LobbyTimeLimit = CFG->GetInt( "bot_lobbytimelimit", 30 );
...
/*
bool InjectDll( DWORD procID, string dll )
{
...
//	cout << hRemoteThread << endl;
	...
	bool res = false;
    if( hRemoteThread )
        res = (bool)WaitForSingleObject( hRemoteThread, 2500 ) != WAIT_TIMEOUT;
...
void CGHost :: ReloadConfig ()
{
	...
	nstmThreadedTalk = CFG->GetInt( "bot_nsthreadedtalk", 0 ) == 0 ? false : true;
	nstmAddTime = CFG->GetInt( "bot_addtime", 0 ) == 0 ? false : true;
	nstmTimeMaxToAdd = CFG->GetInt( "bot_nstmtimemax", 5000 );
	nstmTimeMinToAdd = CFG->GetInt( "bot_nstmtimemin", 2000 );

	m_nscounttrigger = CFG->GetString( "bnet_nscounttrigger", "^" );
	if( m_nscounttrigger.empty( ) )
		m_nscounttrigger = "^";

	nsSendAdminGameStats = CFG->GetInt( "bot_nssendsdmingamestats", 1 ) == 0 ? false : true;

	nsEnableStayPercentageCheck = CFG->GetInt( "bot_nsgamestaycheckenable", 1 ) == 0 ? false : true;
	nsAllowUnknownStayPercentagePlayers = CFG->GetInt( "bot_nsgamestaycheckallownull", 1 ) == 0 ? false : true;
	nsMinGamesCountToApplyRule = CFG->GetInt( "bot_mingamescounttoapplyrule", 2 );
	nsStayRequaredPercentage = CFG->GetInt( "bot_nsgamestaycheckpercentagethreshold", 70 );


	nsEnableStayPercentageCheckNew = CFG->GetInt( "bot_nsenablestaypercentagechecknew", 1 ) == 0 ? false : true;
	nsAllowUnknownStayPercentagePlayersNew = CFG->GetInt( "bot_nsallowunknownstaypercentageplayersnew", 1 ) == 0 ? false : true;
	nsPlayerGamesCountNew = CFG->GetInt( "bot_nsPlayerGamesCountNew", 1 );
	nsStayRequaredPercentage1moreNew = CFG->GetInt( "bot_nsstayrequaredpercentage1morenew", 20 );
	nsStayRequaredPercentage2moreNew = CFG->GetInt( "bot_nsstayrequaredpercentage2morenew", 50 );
	nsStayRequaredPercentage4moreNew = CFG->GetInt( "bot_nsstayrequaredpercentage4morenew", 70 );
	...
	m_LobbyTimeLimit = CFG->GetInt( "bot_lobbytimelimit", 30 );
	m_LobbyTimeLimitMax = CFG->GetInt( "bot_lobbytimelimitmax", 40 );
...
string CGHost :: IncGameNr ( string name)
{
	...
	uint32_t GameNameLen = 0;
	uint32_t GameNumberLen = 0;

	uint32_t idx = GameName.length()-1;
	uint32_t id;
	uint32_t Nr = 0;
	...
	{
		if (idx>=id)
			if (GameName.substr(idx-id, m_nscounttrigger.length() ) == m_nscounttrigger )
			{
				idx = idx-id+1;
				found = true;
				break;
	...
	{
		GameNr = "0";
		//GameName = name;// + " " + m_nscounttrigger;
	}
	else
	{
		GameNr = GameName.substr(idx + m_nscounttrigger.length() - 1, GameName.length()- idx - m_nscounttrigger.length() + 1 );
		GameName = GameName.substr(0, idx /*+ m_nscounttrigger.length()*/ - 2); //хотим получить имя игры без промела и символа номера
	}
	stringstream SS;
	SS << GameNr;
	SS >> Nr;
	...
	GameNumberLen = GameNr.length();
	GameNameLen = GameName.length();

	if ( GameNumberLen + GameNameLen + 1 + m_nscounttrigger.length() > 31)
	{
		GameName = GameName.substr(0, 31 - 1 - m_nscounttrigger.length() - GameNumberLen);
	}

	GameName = GameName + " " + m_nscounttrigger + GameNr;
	return GameName;
}

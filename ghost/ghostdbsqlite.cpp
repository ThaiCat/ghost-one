...
/*uint32_t CGHostDBSQLite :: nsBanCount( uint32_t warn )
{
	uint32_t Count = 0;
	sqlite3_stmt *Statement;
	m_DB->Prepare( "SELECT COUNT(*) FROM bans WHERE warn=? AND (expiredate == '' OR JULIANDAY(expiredate) > JULIANDAY('now'))", (void **)&Statement );

	if( Statement )
	{
		sqlite3_bind_int( Statement, 1, warn );
		int RC = m_DB->Step( Statement );
